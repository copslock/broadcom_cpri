...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  osprey7_v2l8p1_internal_error.c                                   *
...
const char* osprey7_v2l8p1_INTERNAL_e2s_err_code(err_code_t err_code) {
    ...
    case ERR_CODE_RX_TUNING_NOT_DONE: return "ERR_CODE_RX_TUNING_NOT_DONE";
    case ERR_CODE_API_IP_DOES_NOT_MATCH_CORE: return "ERR_CODE_API_IP_DOES_NOT_MATCH_CORE";
    case ERR_CODE_UCODE_IP_DOES_NOT_MATCH_CORE: return "ERR_CODE_UCODE_IP_DOES_NOT_MATCH_CORE";
    case ERR_CODE_REACHED_BUF_SIZE_LIMIT: return "ERR_CODE_REACHED_BUF_SIZE_LIMIT";
...
err_code_t osprey7_v2l8p1_INTERNAL_print_err_msg(srds_access_t *sa__, uint16_t err_code, const char *filename, const char *func_name, uint16_t line_num) {
    if (err_code != ERR_CODE_NONE) {
        USR_PRINTF(("ERROR: SerDes err_code = %s : %d filename = %s, function = %s, linenum=%d\n",osprey7_v2l8p1_INTERNAL_e2s_err_code(err_code),err_code, filename, func_name, line_num));
    }
    return err_code;
}
err_code_t osprey7_v2l8p1_INTERNAL_print_err_msg_and_triage_info(srds_access_t *sa__, uint16_t err_code, const char *filename, const char *func_name, uint16_t line_num) {
    if (err_code != ERR_CODE_NONE) {
        USR_PRINTF(("ERROR:%s->%s() SerDes err_code = %s : %d %d\n", filename, func_name, osprey7_v2l8p1_INTERNAL_e2s_err_code(err_code),err_code, line_num));
        osprey7_v2l8p1_INTERNAL_print_triage_info(sa__, err_code, 1, 1, line_num);
    }
    return err_code;
}
