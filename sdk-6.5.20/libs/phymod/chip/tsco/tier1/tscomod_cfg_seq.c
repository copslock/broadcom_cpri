...
int tscomod_model_num_get(PHYMOD_ST* pc, uint32_t* model_num)
{
...
*/
int tscomod_speed_id_get(PHYMOD_ST* pc, int *speed_id)
{
...
*/
int tscomod_enable_get(PHYMOD_ST* pc, uint32_t* enable)
{
...
one thing.
*/
int tscomod_disable_set(PHYMOD_ST* pc)
{
    SC_X2_CTLr_t reg_sc_ctrl;
...
int tscomod_enable_set(PHYMOD_ST* pc)
{
    SC_X2_CTLr_t reg_sc_ctrl;
...
int tscomod_set_an_timers(PHYMOD_ST* pc, phymod_ref_clk_t refclk, uint32_t *pam4_an_timer_value)    /* SET_AN_TIMERS */
{
...
int tscomod_get_pcs_latched_link_status(PHYMOD_ST* pc, uint32_t *link)
{
...
*/
int tscomod_rsfec_cw_type_set(PHYMOD_ST* pc, tscomod_rs_fec_cw_type  cw_type, int fec_bypass_correction)              /* PMD_X4_RESET */
{
...
int tscomod_osts_pipeline(PHYMOD_ST* pc, uint32_t en)
{
...
int tscomod_osts_pipeline_get(PHYMOD_ST* pc, uint32_t* en)
{
    TX_X2_TX_TS_CTLr_t reg_tx_x2_ts_ctlr;
...
int tscomod_pcs_rx_lane_swap(PHYMOD_ST* pc, int rx_lane_swap)
{
    uint8_t rx_lane_map_physical[8], lane;
    RX_X1_RX_LN_SWPr_t    RX_X1_RX_LN_SWPr_reg;
    int start_lane, num_lane, pml_index;
    uint32_t tmp_rx_lane_map = 0;
    ...
    /* Adjust logical lane for PML1 */
    if (pml_index) {
        for (lane = 0; lane < TSCOMOD_NOF_LANES_IN_CORE; lane++) {
            tmp_rx_lane_map |= ((rx_lane_swap >> (lane * 4)) & 0xf) << (((lane + 4) % 8) * 4);
        }
    } else {
        tmp_rx_lane_map = rx_lane_swap;
    }

    /*first we need to translate logical ln based on physical lane based lane mask*/
    for (lane = 0; lane < TSCOMOD_NOF_LANES_IN_CORE; lane++){
        rx_lane_map_physical[((tmp_rx_lane_map >> (lane * 4 )) & 0xf)] = lane;
    }

    /*first for physcial lane 0 through 3, use mmp0 */
    /* based on the start_lane, decide the U0/U1 */
    if ( start_lane < 4) {
        start_lane = 0;
    } else { 
        start_lane = 4;
    ...
    /* first write U0/U1 mpp0 */
    RX_X1_RX_LN_SWPr_CLR(RX_X1_RX_LN_SWPr_reg);
    pc->lane_mask = 0x1 << (0 + start_lane);
    ...
    /* first write U0/U1 mpp1 */
    RX_X1_RX_LN_SWPr_CLR(RX_X1_RX_LN_SWPr_reg);
    pc->lane_mask = 0x1 << (2 + start_lane);
...
@param rx_lane_swap value tells phy to log lanes mapping
@returns PHYMOD_E_NONE if no errors. PHYMOD_EERROR else.
@details Rx mux select config*/
int tscomod_pcs_rx_mux_select(PHYMOD_ST* pc, int rx_lane_swap)
{
    uint8_t rx_lane_map_physical[8] = {0}, lane;
    PMD_X1_PMD_X1_RX_MUX_SELSr_t PMD_X1_PMD_X1_RX_MUX_SELS_reg;

    PMD_X1_PMD_X1_RX_MUX_SELSr_CLR(PMD_X1_PMD_X1_RX_MUX_SELS_reg);


    /*
     * rx_lane_swap == 0 means setting all the mux selection to 0.
     * So leave rx_lane_map_physical as default values (all 0).
     * Otherwise, we need to translate logical ln based on physical lane based lane mask
     */
    if (rx_lane_swap) {
        for (lane = 0; lane < TSCOMOD_NOF_LANES_IN_CORE; lane++) {
            rx_lane_map_physical[((rx_lane_swap >> (lane * 4)) & 0xf)] = lane / 4;
        }
    }

    /* only PML0 MPP0 copy needs to be programmed */
    pc->lane_mask = 0x1 << 0;
    PMD_X1_PMD_X1_RX_MUX_SELSr_RX_PL0_SELf_SET(PMD_X1_PMD_X1_RX_MUX_SELS_reg, (rx_lane_map_physical[0] & 0x1)); 
    PMD_X1_PMD_X1_RX_MUX_SELSr_RX_PL1_SELf_SET(PMD_X1_PMD_X1_RX_MUX_SELS_reg, (rx_lane_map_physical[1] & 0x1)); 
    PMD_X1_PMD_X1_RX_MUX_SELSr_RX_PL2_SELf_SET(PMD_X1_PMD_X1_RX_MUX_SELS_reg, (rx_lane_map_physical[2] & 0x1)); 
    PMD_X1_PMD_X1_RX_MUX_SELSr_RX_PL3_SELf_SET(PMD_X1_PMD_X1_RX_MUX_SELS_reg, (rx_lane_map_physical[3] & 0x1)); 
    PMD_X1_PMD_X1_RX_MUX_SELSr_RX_PL4_SELf_SET(PMD_X1_PMD_X1_RX_MUX_SELS_reg, (rx_lane_map_physical[4] & 0x1)); 
    PMD_X1_PMD_X1_RX_MUX_SELSr_RX_PL5_SELf_SET(PMD_X1_PMD_X1_RX_MUX_SELS_reg, (rx_lane_map_physical[5] & 0x1)); 
    PMD_X1_PMD_X1_RX_MUX_SELSr_RX_PL6_SELf_SET(PMD_X1_PMD_X1_RX_MUX_SELS_reg, (rx_lane_map_physical[6] & 0x1)); 
    PMD_X1_PMD_X1_RX_MUX_SELSr_RX_PL7_SELf_SET(PMD_X1_PMD_X1_RX_MUX_SELS_reg, (rx_lane_map_physical[7] & 0x1)); 
    PHYMOD_IF_ERR_RETURN(MODIFY_PMD_X1_PMD_X1_RX_MUX_SELSr(pc, PMD_X1_PMD_X1_RX_MUX_SELS_reg)); 
...
@returns PHYMOD_E_NONE if no errors. PHYMOD_EERROR else.
@details Rx lane swap */
int tscomod_pcs_single_rx_lane_swap_set(PHYMOD_ST* pc, int rx_physical_lane, int rx_logical_lane)
{
    RX_X1_RX_LN_SWPr_t    RX_X1_RX_LN_SWPr_reg;
    int start_lane, num_lane, pml_index, tmp_rx_logical_lane = 0;

    tmp_rx_logical_lane = rx_logical_lane;
    ...
    /* Adjust logical lane for PML1 */
    if (pml_index) {
        tmp_rx_logical_lane = (rx_logical_lane + 4) % 8;
    }

    /*first decide which MPP to write */
    if (rx_physical_lane < 4) {
        /* check which PML to write */
        if (pml_index) {
            start_lane = 4;
        } else {
            start_lane = 0;
    ...
    switch (rx_physical_lane) {
        case 0:
        case 4:
            RX_X1_RX_LN_SWPr_PHYSICAL0_TO_LOGICAL_SELf_SET(RX_X1_RX_LN_SWPr_reg, tmp_rx_logical_lane & 0x7);
            break;
        case 1:
        case 5:
            RX_X1_RX_LN_SWPr_PHYSICAL1_TO_LOGICAL_SELf_SET(RX_X1_RX_LN_SWPr_reg, tmp_rx_logical_lane & 0x7);
            break;
        case 2:
        case 6:
            RX_X1_RX_LN_SWPr_PHYSICAL2_TO_LOGICAL_SELf_SET(RX_X1_RX_LN_SWPr_reg, tmp_rx_logical_lane & 0x7);
            break;
        case 3:
        case 7:
            RX_X1_RX_LN_SWPr_PHYSICAL3_TO_LOGICAL_SELf_SET(RX_X1_RX_LN_SWPr_reg, tmp_rx_logical_lane & 0x7);
            break;
        default:
            PHYMOD_DEBUG_ERROR(("Unsupported physical lane \n"));
            return PHYMOD_E_UNAVAIL;
    }
    /* write U0/U1 copy mpp*/
    pc->lane_mask = 0x1 << start_lane;
    PHYMOD_IF_ERR_RETURN(MODIFY_RX_X1_RX_LN_SWPr(pc, RX_X1_RX_LN_SWPr_reg));
...
int tscomod_pcs_tx_m1_lane_swap(PHYMOD_ST* pc, int tx_lane_swap)
{
    uint8_t tx_lane_map_physical[8], lane;
    uint32_t tmp_tx_lane_map_u1 = 0x0, tmp_tx_lane_map;
    TX_X1_TX_LN_SWP_M1r_t TX_X1_TX_LN_SWPr_reg;
    int start_lane, num_lane, pml_index = 0;

    tmp_tx_lane_map = tx_lane_swap; 
    PHYMOD_IF_ERR_RETURN
        (phymod_util_lane_config_get(pc, &start_lane, &num_lane));
    ...
    /* need to adjust the logical lane if it's for PML1 */
    if (pml_index) {
        for (lane = 0; lane < TSCOMOD_NOF_LANES_IN_CORE; lane++) {
            tmp_tx_lane_map_u1 |= ((tx_lane_swap >> (lane * 4)) & 0xf) << (((lane + 4) % 8) * 4);
        }
        tmp_tx_lane_map = tmp_tx_lane_map_u1; 
    }
                
    /*first we need to translate logical ln based on physical lane based lane mask*/
    for (lane = 0; lane < TSCOMOD_NOF_LANES_IN_CORE; lane++) {
        tx_lane_map_physical[((tmp_tx_lane_map >> (lane * 4)) & 0xf)] = lane;
    }

    /* next need to adjust the MPP1 logical lane offset */
    for (lane = 0; lane < TSCOMOD_NOF_LANES_IN_CORE / 2; lane++) {
    ...
    TX_X1_TX_LN_SWP_M1r_CLR(TX_X1_TX_LN_SWPr_reg);
    if (pml_index) {
        pc->lane_mask = 0x1 << 4;
    } else {
        pc->lane_mask = 0x1 << 0;
    }
    TX_X1_TX_LN_SWP_M1r_M1_LOGICAL_TO_PHYSICAL0_SELf_SET(TX_X1_TX_LN_SWPr_reg, (tx_lane_map_physical[0] & 0x7));
    TX_X1_TX_LN_SWP_M1r_M1_LOGICAL_TO_PHYSICAL1_SELf_SET(TX_X1_TX_LN_SWPr_reg, (tx_lane_map_physical[1] & 0x7));
    TX_X1_TX_LN_SWP_M1r_M1_LOGICAL_TO_PHYSICAL2_SELf_SET(TX_X1_TX_LN_SWPr_reg, (tx_lane_map_physical[2] & 0x7));
    TX_X1_TX_LN_SWP_M1r_M1_LOGICAL_TO_PHYSICAL3_SELf_SET(TX_X1_TX_LN_SWPr_reg, (tx_lane_map_physical[3] & 0x7));
    PHYMOD_IF_ERR_RETURN(MODIFY_TX_X1_TX_LN_SWP_M1r(pc, TX_X1_TX_LN_SWPr_reg));
    ...
    TX_X1_TX_LN_SWP_M1r_M1_LOGICAL_TO_PHYSICAL0_SELf_SET(TX_X1_TX_LN_SWPr_reg, (tx_lane_map_physical[4] & 0x7));
    TX_X1_TX_LN_SWP_M1r_M1_LOGICAL_TO_PHYSICAL1_SELf_SET(TX_X1_TX_LN_SWPr_reg, (tx_lane_map_physical[5] & 0x7));
    TX_X1_TX_LN_SWP_M1r_M1_LOGICAL_TO_PHYSICAL2_SELf_SET(TX_X1_TX_LN_SWPr_reg, (tx_lane_map_physical[6] & 0x7));
    TX_X1_TX_LN_SWP_M1r_M1_LOGICAL_TO_PHYSICAL3_SELf_SET(TX_X1_TX_LN_SWPr_reg, (tx_lane_map_physical[7] & 0x7));
    PHYMOD_IF_ERR_RETURN(MODIFY_TX_X1_TX_LN_SWP_M1r(pc, TX_X1_TX_LN_SWPr_reg));
...
@returns PHYMOD_E_NONE if no errors. PHYMOD_EERROR else.
@details Tx lane swap */
int tscomod_pcs_tx_m0_lane_swap(PHYMOD_ST* pc, int tx_lane_swap)
{
    uint8_t tx_lane_map_physical[8], lane;
    uint32_t tmp_tx_lane_map_u1 = 0x0, tmp_tx_lane_map;
    ...
    /* need to adjuts the logical lane if it's for PML1 */ 
    if (pml_index) {
        for (lane = 0; lane < TSCOMOD_NOF_LANES_IN_CORE; lane++) {
            tmp_tx_lane_map_u1 |= ((tx_lane_swap >> (lane * 4)) & 0xf) << (((lane + 4) % 8) * 4);
        }
    ...
    /*first we need to translate logical ln based on physical lane based lane mask*/
    for (lane = 0; lane < TSCOMOD_NOF_LANES_IN_CORE; lane++) {
        tx_lane_map_physical[((tmp_tx_lane_map >> (lane * 4)) & 0xf)] = lane > 3 ? lane*2 - 7 : lane*2;
    }
...
@param tx_lane_swap value tells phy to log lanes mapping
@returns PHYMOD_E_NONE if no errors. PHYMOD_EERROR else.
@details Tx mux select config*/
int tscomod_pcs_tx_mux_select(PHYMOD_ST* pc, int tx_lane_swap)
{
    uint8_t tx_lane_map_physical[8] = {0}, lane;
    PMD_X1_PMD_X1_TX_MUX_SELSr_t PMD_X1_PMD_X1_TX_MUX_SELS_reg;

    PMD_X1_PMD_X1_TX_MUX_SELSr_CLR(PMD_X1_PMD_X1_TX_MUX_SELS_reg);

    /*
     * tx_lane_swap == 0 means setting all the mux selection to 0.
     * So leave tx_lane_map_physical as default values (all 0).
     * Otherwise, we need to translate logical ln based on physical lane based lane mask
     */
    if (tx_lane_swap) {
        for (lane = 0; lane < TSCOMOD_NOF_LANES_IN_CORE; lane++) {
            tx_lane_map_physical[((tx_lane_swap >> (lane * 4)) & 0xf)] = lane / 4;
        }
    }

    /* only PML0 MPP0 copy needs to be programmed */
    pc->lane_mask = 0x1 << 0;
    PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL0_SELf_SET(PMD_X1_PMD_X1_TX_MUX_SELS_reg, (tx_lane_map_physical[0] & 0x1)); 
    PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL1_SELf_SET(PMD_X1_PMD_X1_TX_MUX_SELS_reg, (tx_lane_map_physical[1] & 0x1)); 
    PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL2_SELf_SET(PMD_X1_PMD_X1_TX_MUX_SELS_reg, (tx_lane_map_physical[2] & 0x1)); 
    PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL3_SELf_SET(PMD_X1_PMD_X1_TX_MUX_SELS_reg, (tx_lane_map_physical[3] & 0x1)); 
    PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL4_SELf_SET(PMD_X1_PMD_X1_TX_MUX_SELS_reg, (tx_lane_map_physical[4] & 0x1)); 
    PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL5_SELf_SET(PMD_X1_PMD_X1_TX_MUX_SELS_reg, (tx_lane_map_physical[5] & 0x1)); 
    PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL6_SELf_SET(PMD_X1_PMD_X1_TX_MUX_SELS_reg, (tx_lane_map_physical[6] & 0x1)); 
    PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL7_SELf_SET(PMD_X1_PMD_X1_TX_MUX_SELS_reg, (tx_lane_map_physical[7] & 0x1)); 
    PHYMOD_IF_ERR_RETURN(MODIFY_PMD_X1_PMD_X1_TX_MUX_SELSr(pc, PMD_X1_PMD_X1_TX_MUX_SELS_reg)); 
...
@returns PHYMOD_E_NONE if no errors. PHYMOD_EERROR else.
@details Tx lane swap */
int tscomod_pcs_single_tx_lane_swap_set(PHYMOD_ST* pc, int tx_physical_lane, int tx_logical_lane)
{
    TX_X1_TX_LN_SWP_M1r_t TX_X1_TX_LN_SWPr_reg;
    int start_lane, num_lane, pml_index, tmp_tx_logical_lane = 0;

    tmp_tx_logical_lane = tx_logical_lane;
    ...
    /* Adjust logical lane for PML1 */
    if (pml_index) {
        tmp_tx_logical_lane = (tmp_tx_logical_lane + 4) % 8;
    }
    ...
    switch (tx_physical_lane) {
        case 0:
        case 4:
            TX_X1_TX_LN_SWP_M1r_M1_LOGICAL_TO_PHYSICAL0_SELf_SET(TX_X1_TX_LN_SWPr_reg, tmp_tx_logical_lane & 0x7);
            break;
        case 1:
        case 5:
            TX_X1_TX_LN_SWP_M1r_M1_LOGICAL_TO_PHYSICAL1_SELf_SET(TX_X1_TX_LN_SWPr_reg, tmp_tx_logical_lane & 0x7);
            break;
        case 2:
        case 6:
            TX_X1_TX_LN_SWP_M1r_M1_LOGICAL_TO_PHYSICAL2_SELf_SET(TX_X1_TX_LN_SWPr_reg, tmp_tx_logical_lane & 0x7);
            break;
        case 3:
        case 7:
            TX_X1_TX_LN_SWP_M1r_M1_LOGICAL_TO_PHYSICAL3_SELf_SET(TX_X1_TX_LN_SWPr_reg, tmp_tx_logical_lane & 0x7);
            break;
        default:
            PHYMOD_DEBUG_ERROR(("Unsupported physical lane \n"));
            return PHYMOD_E_UNAVAIL;
...
int tscomod_pcs_rx_scramble_idle_en(PHYMOD_ST* pc, int en)
{
...
int tscomod_pcs_rx_ts_en_get(PHYMOD_ST* pc, uint32_t* en)
{
 ...
 */
int tscomod_synce_mode_set(PHYMOD_ST* pc, int stage0_mode, int stage1_mode)
{
...
int tscomod_synce_mode_get(PHYMOD_ST* pc, int* stage0_mode, int* stage1_mode)
{
    INTEGER_DIVr_t INTEGER_DIVr_reg;
...
int tscomod_synce_clk_ctrl_set(PHYMOD_ST* pc, uint32_t val)
{
...
int tscomod_synce_clk_ctrl_get(PHYMOD_ST* pc, uint32_t* val)
{
    FRACTIONAL_DIVr_t FRACTIONAL_DIVr_reg;
...
int tscomod_resolved_port_mode_get(PHYMOD_ST* pc, uint32_t* port_mode)
{
...
int tscomod_revid_get(PHYMOD_ST* pc, uint32_t* rev_id)
{
    MAIN0_SERDESIDr_t MAIN0_SERDESIDr_reg;
...
int tscomod_fec_arch_decode_get(int fec_arch, phymod_fec_type_t* fec_type)
{
    switch (fec_arch) {
        case 1:
...
int tscomod_fec_override_set(PHYMOD_ST* pc, uint32_t enable)
{
    ...
    temp_reg_value &= ~TSCOMOD_FEC_OVERRIDE_MASK;
    temp_reg_value |= (enable & TSCOMOD_FEC_OVERRIDE_MASK);
...
int tscomod_fec_override_get(PHYMOD_ST* pc, uint32_t* enable)
{
    SC_X2_SW_SPARE1r_t SC_X2_SW_SPARE1r_reg;
    uint16_t temp_reg_value;
    ...
    PHYMOD_IF_ERR_RETURN(READ_SC_X2_SW_SPARE1r(pc, &SC_X2_SW_SPARE1r_reg));
    temp_reg_value = SC_X2_SW_SPARE1r_GET(SC_X2_SW_SPARE1r_reg);
    *enable = temp_reg_value & TSCOMOD_FEC_OVERRIDE_MASK;

    return PHYMOD_E_NONE;
}


int tscomod_fec_correctable_counter_get(PHYMOD_ST* pc, uint32_t* count)
{
...
int tscomod_fec_uncorrectable_counter_get(PHYMOD_ST* pc, uint32_t* count)
{
    int start_lane, num_lane;
...
int tscomod_pmd_reset_seq(PHYMOD_ST* pc) /* PMD_RESET_SEQ */
{
...
int tscomod_set_an_port_mode(PHYMOD_ST* pc, int starting_lane)    /* SET_AN_PORT_MODE */
{
    uint16_t new_port_mode_sel = TSCOMOD_DUAL_PORT;
    ...
    if (port_mode_sel_reg == TSCOMOD_MULTI_MPP_PORT) {
        is_multi_mpp_mode = 1;
    }

    phy_copy.lane_mask = 0x1 << starting_lane;
    ...
    /* if multi MPPs need to write MPP1 as well */
    if (is_multi_mpp_mode) {
        phy_copy.lane_mask = 0x1 << (starting_lane + 2);
        PHYMOD_IF_ERR_RETURN (MODIFY_MAIN0_SETUPr(&phy_copy, MAIN0_SETUPr_reg));
    }
...
@returns The value PHYMOD_E_NONE upon successful completion
*/
int tscomod_update_port_mode(PHYMOD_ST *pc, uint32_t data_rate)
{
    MAIN0_SETUPr_t mode_reg;
    PHYMOD_ST phy_copy;
    int port_mode_sel = 0, mpp_index = 0, pml_index = 0;
    uint32_t multi_mpp_port_mode = 0;
    ...
    } else if (pc->lane_mask == 0x0f) {
        mpp_index = 0;
        pml_index = 0;
        /* For 4-lane port, if data rate greater than 200G, then use multi_mpp
         * mode. Otherwise, use single_port mode.
         */
        if (data_rate > 200000) {
            multi_mpp_port_mode = 1;
        }
    } else if (pc->lane_mask == 0xf0) {
        pml_index = 1;
        mpp_index = 0;
        /* For 4-lane port, if data rate greater than 200G, then use multi_mpp
         * mode. Otherwise, use single_port mode.
    ...
    } else if (pc->lane_mask & 0x3) {
        mpp_index = 0;
        pml_index = 0;
    } else if (pc->lane_mask & 0xc) {
        mpp_index = 1;
        pml_index = 0;
    } else if (pc->lane_mask & 0x30) {
        mpp_index = 0;
        pml_index = 1;
    } else if (pc->lane_mask & 0xc0) {
        mpp_index = 1;
        pml_index = 1;
    }

    /* next set the PML/ MPP index properly */
    phy_copy.lane_mask = (0x1 << (mpp_index * 2)) << (pml_index * 4);

    if (!multi_mpp_port_mode) {
        if ((pc->lane_mask == 0x3) || (pc->lane_mask == 0xc) ||
            (pc->lane_mask == 0x30) || (pc->lane_mask == 0xc0) ||
            (pc->lane_mask == 0xf) || (pc->lane_mask == 0xf0)) {
            port_mode_sel = TSCOMOD_SINGLE_PORT;
        } else {
            port_mode_sel = TSCOMOD_DUAL_PORT;
        }
    }

    if (multi_mpp_port_mode) {
        port_mode_sel = TSCOMOD_MULTI_MPP_PORT;
        ...
        if (pml_index == 0) {
            phy_copy.lane_mask = 0x1 << 2;
        } else if (pml_index == 1) {
            phy_copy.lane_mask = 0x1 << 6;
        }
        PHYMOD_IF_ERR_RETURN(MODIFY_MAIN0_SETUPr(&phy_copy, mode_reg));
    } else {
        MAIN0_SETUPr_PORT_MODE_SELf_SET(mode_reg, port_mode_sel);
        PHYMOD_IF_ERR_RETURN(MODIFY_MAIN0_SETUPr(&phy_copy, mode_reg));
        if (pc->lane_mask == 0xf) {
            /* Set MPP1 to DUAL_PORT mode (default) since it's not used. */
            phy_copy.lane_mask = 0x1 << 2;
            port_mode_sel = TSCOMOD_DUAL_PORT;
            MAIN0_SETUPr_PORT_MODE_SELf_SET(mode_reg, port_mode_sel);
            PHYMOD_IF_ERR_RETURN(MODIFY_MAIN0_SETUPr(&phy_copy, mode_reg));
...
int tscomod_refclk_set(PHYMOD_ST* pc, tscomod_refclk_t ref_clk)
{
    MAIN0_SETUPr_t MAIN0_SETUPr_reg;
    int start_lane, num_lane;
    PHYMOD_IF_ERR_RETURN
...
int tscomod_refclk_get(PHYMOD_ST* pc, tscomod_refclk_t* ref_clk)
{
...
*/
int tscomod_pmd_x4_reset(PHYMOD_ST* pc)              /* PMD_X4_RESET */
{
    phymod_access_t pa_copy;
    ...
    /*
     * If the port is 8-lane port, set the register access flag
     * when accessing 4-copy PCS register.
     */
    if (pa_copy.lane_mask == 0xff) {
        pa_copy.lane_mask |= 0x1 << PHYMOD_TSCO_8_LANE_PORT_SHIFT;
    }
...
int tscomod_set_sc_speed(PHYMOD_ST* pc, int mapped_speed, int set_sw_speed_change)  /* SET_SC_SPEED */
{
...
*/
int tscomod_autoneg_control(PHYMOD_ST* pc, tscomod_an_control_t *an_control)
{
    phymod_access_t pa_copy;
    ...
    /* Set OUI */
    if (an_control->an_type != TSCOMOD_AN_MODE_CL73) {
        if (start_lane < 2) {
            pa_copy.lane_mask = 0x1;
        } else if  (start_lane < 4) {
            pa_copy.lane_mask = 0x4;
        } else if  (start_lane < 6) {
            pa_copy.lane_mask = 0x10;
        } else {
            pa_copy.lane_mask = 0x40;
        }
...
int tscomod_autoneg_control_get(PHYMOD_ST* pc, tscomod_an_control_t *an_control, int *an_complete)
{
    AN_X2_CL73_CFGr_t      AN_X2_CL73_CFGr_reg;
    ...
    if (AN_X2_CL73_CFGr_CL73_BAM_ENABLEf_GET(AN_X2_CL73_CFGr_reg) == 1) {
        if (AN_X2_CL73_CFGr_MSA_FEC_MAPPINGf_GET(AN_X2_CL73_CFGr_reg) == 1) {
            ...
            if (!(base_ability1) && !(base_ability2)) {
                an_control->an_type = TSCOMOD_AN_MODE_MSA;
                an_control->enable = 1;
            } else {
        ...
        an_control->enable = 1;
         ...
         an_control->an_type = TSCOMOD_AN_MODE_NONE;
         an_control->enable = 0;
    }
...
int tscomod_autoneg_status_get(PHYMOD_ST* pc, int *an_en, int *an_done)
{
    AN_X2_CL73_CFGr_t  AN_X2_CL73_CFGr_reg;
    AN_X2_AN_MISC_STSr_t  AN_X2_AN_MISC_STSr_reg;
...
int tscomod_autoneg_fec_status_get(PHYMOD_ST* pc, uint8_t *fec_status)
{
...
*/
int tscomod_autoneg_ability_set(PHYMOD_ST* pc,
    ...
    for (i = 0; i < autoneg_abilities->num_abilities; i++) {
        ...
        case 100000:
            if (an_ability->an_mode == phymod_AN_MODE_CL73) {
                if (an_ability->resolved_num_lanes == 1) {
                    /* CL73 100G 1 lane */
                    AN_X2_LD_BASE_ABIL2r_BASE_100G_CR1_KR1f_SET(AN_X2_ABILITIES_LD_BASE_ABILITIES_2r_reg, 1);
                } else if (an_ability->resolved_num_lanes == 2) {
                    /* CL73-100G-2lanes */
                ...
                if (an_ability->resolved_num_lanes == 1) {
                    /* CL73BAM 100G-1lane */
                    AN_X2_LD_UP1_ABIL1r_BAM_100G_CR1_KR1f_SET(AN_X2_ABILITIES_LD_UP1_ABILITIES_1r_reg, 1);
                } else if (an_ability->resolved_num_lanes == 2) {
                    /* CL73BAM-100G-2lanes */
        ...
        case 200000:
            if (an_ability->an_mode == phymod_AN_MODE_CL73) {
                if (an_ability->resolved_num_lanes == 2) {
                    /* 200G 2 lane CL73 */
                    AN_X2_LD_BASE_ABIL2r_BASE_200G_CR2_KR2f_SET(AN_X2_ABILITIES_LD_BASE_ABILITIES_2r_reg, 1);
                } else {
                    /* CL73-200G-4lanes */
        ...
        case 400000:
            {
                if (an_ability->resolved_num_lanes == 4) {
                    AN_X2_LD_BASE_ABIL2r_BASE_400G_CR4_KR4f_SET(AN_X2_ABILITIES_LD_BASE_ABILITIES_2r_reg, 1);
                } else {
...
int _tscomod_an_ability_construct(phymod_autoneg_advert_ability_t* an_ability,
                          uint32_t speed,
                          int num_lanes,
 ...
 */
int tscomod_autoneg_ability_get(PHYMOD_ST* pc, phymod_autoneg_advert_abilities_t* autoneg_abilities)
{
    AN_X2_LD_BASE_ABIL1r_t        AN_X2_LD_BASE_ABIL1r_reg;
    ...
    if (AN_X2_LD_BASE_ABIL1r_BASE_100G_KR4f_GET(AN_X2_LD_BASE_ABIL1r_reg) ||
        ...
        PHYMOD_IF_ERR_RETURN(
          _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
    ...
    if (AN_X2_LD_BASE_ABIL2r_BASE_400G_CR4_KR4f_GET(AN_X2_LD_BASE_ABIL2r_reg)) {
        PHYMOD_IF_ERR_RETURN(
         _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
    ...
    if (AN_X2_CL73_CFGr_MSA_FEC_MAPPINGf_GET(AN_X2_CL73_CFGr_reg)) {
        ...
        if (AN_X2_LD_UP1_ABIL1r_BAM_100G_CR1_KR1f_GET(AN_X2_LD_UP1_ABIL1r_reg)) {
            PHYMOD_IF_ERR_RETURN(
              _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                    100000, 1, phymod_fec_RS544, pause, phymod_channel_long, phymod_AN_MODE_CL73BAM));
            i++;
        }
    ...
    if (AN_X2_LD_BASE_ABIL2r_BASE_200G_CR2_KR2f_GET(AN_X2_LD_BASE_ABIL2r_reg)) {
        PHYMOD_IF_ERR_RETURN(
            _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
    ...
    if (AN_X2_LD_BASE_ABIL2r_BASE_100G_CR1_KR1f_GET(AN_X2_LD_BASE_ABIL2r_reg)) {
        PHYMOD_IF_ERR_RETURN(
            _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
...
int tscomod_autoneg_remote_ability_get(PHYMOD_ST* pc, phymod_autoneg_advert_abilities_t *autoneg_abilities)
{
    ...
    /* Get Pause Ability */
    pause = (base_0 >> TSCOMOD_AN_BASE0_PAGE_PAUSE_OFFSET) & TSCOMOD_AN_BASE0_PAGE_PAUSE_MASK;
    fec_cl74 = (base_2 >> TSCOMOD_AN_BASE2_CL74_ABILITY_REQ_SUP_OFFSET) & TSCOMOD_AN_BASE2_CL74_ABILITY_REQ_SUP_MASK;

    /* Get Base Abilities */
    if (((base_1 >> TSCOMOD_AN_BASE1_TECH_ABILITY_100G_KR4_OFFSET) & TSCOMOD_AN_BASE1_TECH_ABILITY_100G_KR4_MASK) ||
        ((base_1 >> TSCOMOD_AN_BASE1_TECH_ABILITY_100G_CR4_OFFSET) & TSCOMOD_AN_BASE1_TECH_ABILITY_100G_CR4_MASK)) {
        PHYMOD_IF_ERR_RETURN(
          _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                100000, 4, phymod_fec_CL91, pause, phymod_channel_long, phymod_AN_MODE_CL73));
        if ((base_1 >> TSCOMOD_AN_BASE1_TECH_ABILITY_100G_CR4_OFFSET) & TSCOMOD_AN_BASE1_TECH_ABILITY_100G_CR4_MASK) {
            autoneg_abilities->autoneg_abilities[i].medium = phymodFirmwareMediaTypeCopperCable;
        }
        i++;
    }
    if (((base_1 >> TSCOMOD_AN_BASE1_TECH_ABILITY_40G_KR4_OFFSET) & TSCOMOD_AN_BASE1_TECH_ABILITY_40G_KR4_MASK) ||
        ((base_1 >> TSCOMOD_AN_BASE1_TECH_ABILITY_40G_CR4_OFFSET) & TSCOMOD_AN_BASE1_TECH_ABILITY_40G_CR4_MASK)) {
        if (fec_cl74 == 0x3) {
            PHYMOD_IF_ERR_RETURN(
              _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
        ...
        if ((base_1 >> TSCOMOD_AN_BASE1_TECH_ABILITY_40G_CR4_OFFSET) & TSCOMOD_AN_BASE1_TECH_ABILITY_40G_CR4_MASK) {
            autoneg_abilities->autoneg_abilities[i].medium = phymodFirmwareMediaTypeCopperCable;
        }
        i++;
    }
    fec_cl74 = (base_2 >> TSCOMOD_AN_BASE2_25G_BASE_R_FEC_ABILITY_REQ_OFFSET) & TSCOMOD_AN_BASE2_25G_BASE_R_FEC_ABILITY_REQ_MASK;
    fec_cl91 = (base_2 >> TSCOMOD_AN_BASE2_25G_RS_FEC_ABILITY_REQ_OFFSET) & TSCOMOD_AN_BASE2_25G_RS_FEC_ABILITY_REQ_MASK;
    if ((base_1 >> TSCOMOD_AN_BASE1_TECH_ABILITY_25G_KR1_CR1_OFFSET) & TSCOMOD_AN_BASE1_TECH_ABILITY_25G_KR1_CR1_MASK) {
    ...
    if ((base_1 >> TSCOMOD_AN_BASE1_TECH_ABILITY_25G_KRS1_CRS1_OFFSET) & TSCOMOD_AN_BASE1_TECH_ABILITY_25G_KRS1_CRS1_MASK) {
        if (fec_cl74) {
            PHYMOD_IF_ERR_RETURN(
              _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
    ...
    fec_cl74 = (base_2 >> TSCOMOD_AN_BASE2_CL74_ABILITY_REQ_SUP_OFFSET) & TSCOMOD_AN_BASE2_CL74_ABILITY_REQ_SUP_MASK;
    if ((base_1 >> TSCOMOD_AN_BASE1_TECH_ABILITY_10G_KR1_OFFSET) & TSCOMOD_AN_BASE1_TECH_ABILITY_10G_KR1_MASK) {
        if (fec_cl74 == 0x3) {
            PHYMOD_IF_ERR_RETURN(
              _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
    ...
    if ((base_0 >> TSCOMOD_AN_BASE0_PAGE_NP_OFFSET) & TSCOMOD_AN_BASE0_PAGE_NP_MASK) {
        msa_code_13_23 = (mp5_1 >> TSCOMOD_AN_MSG_PAGE1_OUI_13to23_OFFSET) & TSCOMOD_AN_MSG_PAGE1_OUI_13to23_MASK;
        msa_code_2_12 = (mp5_2 >> TSCOMOD_AN_MSG_PAGE2_OUI_2to12_OFFSET) & TSCOMOD_AN_MSG_PAGE2_OUI_2to12_MASK;
        msa_code_0_1 = (up1_0 >> TSCOMOD_AN_UF_PAGE0_OUI_OFFSET) & TSCOMOD_AN_UF_PAGE0_OUI_MASK;
        bam_code = (up1_0 >> TSCOMOD_AN_UF_PAGE0_UD_0to8_OFFSET) & TSCOMOD_AN_UF_PAGE0_UD_0to8_MASK;

        if ((msa_code_13_23 == TSCOMOD_MSA_OUI_13to23) && (msa_code_2_12 == TSCOMOD_MSA_OUI_2to12) && (msa_code_0_1 == TSCOMOD_MSA_OUI_0to1)) {
            /* UF page is MSA page */
            fec_cl74 = (up1_2 >> TSCOMOD_AN_UF_PAGE2_MSA_CL74_REQ_OFFSET) & TSCOMOD_AN_UF_PAGE2_MSA_CL74_REQ_MASK;
            fec_cl91 = (up1_2 >> TSCOMOD_AN_UF_PAGE2_MSA_CL91_REQ_OFFSET) & TSCOMOD_AN_UF_PAGE2_MSA_CL91_REQ_MASK;
            if (((up1_1 >> TSCOMOD_AN_UF_PAGE1_MSA_50G_CR2_ABILITY_OFFSET) & TSCOMOD_AN_UF_PAGE1_MSA_50G_CR2_ABILITY_MASK) ||
                ((up1_1 >> TSCOMOD_AN_UF_PAGE1_MSA_50G_KR2_ABILITY_OFFSET) & TSCOMOD_AN_UF_PAGE1_MSA_50G_KR2_ABILITY_MASK)) {
                if (fec_cl74 || fec_cl91) {
                    if(fec_cl74) {
                        PHYMOD_IF_ERR_RETURN(
                           _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
            ...
            if (((up1_1 >> TSCOMOD_AN_UF_PAGE1_MSA_25G_KR1_ABILITY_OFFSET) & TSCOMOD_AN_UF_PAGE1_MSA_25G_KR1_ABILITY_MASK) ||
                ((up1_1 >> TSCOMOD_AN_UF_PAGE1_MSA_25G_CR1_ABILITY_OFFSET) & TSCOMOD_AN_UF_PAGE1_MSA_25G_CR1_ABILITY_MASK)) {
                if (fec_cl74 || fec_cl91) {
                    if(fec_cl74) {
                        PHYMOD_IF_ERR_RETURN(
                           _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                                 25000, 1, phymod_fec_CL74, pause, phymod_channel_long, phymod_AN_MODE_MSA));
                        if ((up1_1 >> TSCOMOD_AN_UF_PAGE1_MSA_25G_CR1_ABILITY_OFFSET) & TSCOMOD_AN_UF_PAGE1_MSA_25G_CR1_ABILITY_MASK) {
                            autoneg_abilities->autoneg_abilities[i].medium = phymodFirmwareMediaTypeCopperCable;
                        }
                        i++;
                    }
            ...
            if ((up1_2 >> TSCOMOD_AN_UF_PAGE2_MSA_LFR_OFFSET) & TSCOMOD_AN_UF_PAGE2_MSA_LFR_MASK) {
                /* 50g-1_lane */
                if (((up1_2 >> TSCOMOD_AN_UF_PAGE2_MSA_LF1_OFFSET) & TSCOMOD_AN_UF_PAGE2_MSA_LF1_MASK) &&
                    ((base_2 >> TSCOMOD_AN_BASE2_TECH_ABILITY_50G_KR1_CR1_OFFSET) & TSCOMOD_AN_BASE2_TECH_ABILITY_50G_KR1_CR1_MASK)) {
                    PHYMOD_IF_ERR_RETURN(
                        _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                ...
                /* 100g-2_lane */
                if (((up1_2 >> TSCOMOD_AN_UF_PAGE2_MSA_LF2_OFFSET) & TSCOMOD_AN_UF_PAGE2_MSA_LF2_MASK) &&
                    ((base_2 >> TSCOMOD_AN_BASE2_TECH_ABILITY_100G_KR2_CR2_OFFSET) & TSCOMOD_AN_BASE2_TECH_ABILITY_100G_KR2_CR2_MASK)) {
                    PHYMOD_IF_ERR_RETURN(
                        _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                ...
                /* 200g-4_lane */
                if (((up1_2 >> TSCOMOD_AN_UF_PAGE2_MSA_LF3_OFFSET) & TSCOMOD_AN_UF_PAGE2_MSA_LF3_MASK) &&
                    ((base_2 >> TSCOMOD_AN_BASE2_TECH_ABILITY_200G_KR4_CR4_OFFSET) & TSCOMOD_AN_BASE2_TECH_ABILITY_200G_KR4_CR4_MASK)) {
                    PHYMOD_IF_ERR_RETURN(
                        _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
            ...
            if ((up1_2 >> TSCOMOD_AN_UF_PAGE2_MSA_400G_ABILITY_OFFSET) & TSCOMOD_AN_UF_PAGE2_MSA_400G_ABILITY_MASK) {
                PHYMOD_IF_ERR_RETURN(
                    _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
        ...
        } else if (bam_code == TSCOMOD_BRCM_BAM_CODE) {
            /* UF page is BAM page */
            fec_cl74 = (up1_2 >> TSCOMOD_AN_UF_PAGE2_BAM_CL74_REQ_OFFSET) & TSCOMOD_AN_UF_PAGE2_BAM_CL74_REQ_MASK;
            fec_cl91 = (up1_2 >> TSCOMOD_AN_UF_PAGE2_BAM_CL91_REQ_OFFSET) & TSCOMOD_AN_UF_PAGE2_BAM_CL91_REQ_MASK;
            if ((up1_2 >> TSCOMOD_AN_UF_PAGE2_BAM_200G_BRCM_KR4_CR4_OFFSET) & TSCOMOD_AN_UF_PAGE2_BAM_200G_BRCM_KR4_CR4_MASK) {
                PHYMOD_IF_ERR_RETURN(
                  _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                        200000, 4, phymod_fec_RS544, pause, phymod_channel_long, phymod_AN_MODE_CL73BAM));
                i++;
            }
            if ((up1_2 >> TSCOMOD_AN_UF_PAGE2_BAM_200G_BRCM_NO_FEC_KR4_CR4_OFFSET) & TSCOMOD_AN_UF_PAGE2_BAM_200G_BRCM_NO_FEC_KR4_CR4_MASK) {
                PHYMOD_IF_ERR_RETURN(
                  _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                        200000, 4, phymod_fec_None, pause, phymod_channel_long, phymod_AN_MODE_CL73BAM));
                i++;
            }
            if ((up1_2 >> TSCOMOD_AN_UF_PAGE2_BAM_100G_BRCM_FEC_528_KR2_CR2_OFFSET) & TSCOMOD_AN_UF_PAGE2_BAM_100G_BRCM_FEC_528_KR2_CR2_MASK) {
                PHYMOD_IF_ERR_RETURN(
                  _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                        100000, 2, phymod_fec_CL91, pause, phymod_channel_long, phymod_AN_MODE_CL73BAM));
                i++;
            }
            if ((up1_2 >> TSCOMOD_AN_UF_PAGE2_BAM_100G_BRCM_NO_FEC_KR2_CR2_OFFSET) & TSCOMOD_AN_UF_PAGE2_BAM_100G_BRCM_NO_FEC_KR2_CR2_MASK) {
                PHYMOD_IF_ERR_RETURN(
                   _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                         100000, 2, phymod_fec_None, pause, phymod_channel_long, phymod_AN_MODE_CL73BAM));
                i++;
            }
            if ((up1_2 >> TSCOMOD_AN_UF_PAGE2_BAM_100G_BRCM_KR4_CR4_OFFSET) & TSCOMOD_AN_UF_PAGE2_BAM_100G_BRCM_KR4_CR4_MASK) {
                PHYMOD_IF_ERR_RETURN(
                   _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                         100000, 4, phymod_fec_RS544, pause, phymod_channel_long, phymod_AN_MODE_CL73BAM));
                i++;
            }
            if ((up1_2 >> TSCOMOD_AN_UF_PAGE2_BAM_100G_BRCM_NO_FEC_X4_OFFSET) & TSCOMOD_AN_UF_PAGE2_BAM_100G_BRCM_NO_FEC_X4_MASK) {
                PHYMOD_IF_ERR_RETURN(
                  _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                        100000, 4, phymod_fec_None, pause, phymod_channel_long, phymod_AN_MODE_CL73BAM));
                i++;
            }
            if ((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_50G_BRCM_FEC_528_CR1_KR1_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_50G_BRCM_FEC_528_CR1_KR1_MASK) {
                PHYMOD_IF_ERR_RETURN(
                  _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                        50000, 1, phymod_fec_CL91, pause, phymod_channel_long, phymod_AN_MODE_CL73BAM));
                i++;
            }
            if ((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_50G_BRCM_NO_FEC_CR1_KR1_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_50G_BRCM_NO_FEC_CR1_KR1_MASK) {
                PHYMOD_IF_ERR_RETURN(
                  _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                        50000, 1, phymod_fec_None, pause, phymod_channel_long, phymod_AN_MODE_CL73BAM));
                i++;
            }
            if ((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_50G_BRCM_FEC_544_CR2_KR2_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_50G_BRCM_FEC_544_CR2_KR2_MASK) {
                PHYMOD_IF_ERR_RETURN(
                  _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                        50000, 2, phymod_fec_RS544, pause, phymod_channel_long, phymod_AN_MODE_CL73BAM));
                i++;
            }
            if (((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_50G_KR2_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_50G_KR2_MASK) ||
                ((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_50G_CR2_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_50G_CR2_MASK)) {
                if (fec_cl74 || fec_cl91) {
                    if (fec_cl74) {
                        PHYMOD_IF_ERR_RETURN(
                          _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                                50000, 2, phymod_fec_CL74, pause, phymod_channel_long, phymod_AN_MODE_CL73BAM));
                        if ((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_50G_CR2_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_50G_CR2_MASK) {
                          autoneg_abilities->autoneg_abilities[i].medium = phymodFirmwareMediaTypeCopperCable;
                        }
                        i++;
                    }
            ...
            if (((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_40G_KR2_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_40G_KR2_MASK) ||
                ((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_40G_CR2_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_40G_CR2_MASK)) {
                if (fec_cl74) {
                    PHYMOD_IF_ERR_RETURN(
                      _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                ...
                if ((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_40G_CR2_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_40G_CR2_MASK) {
                    autoneg_abilities->autoneg_abilities[i].medium = phymodFirmwareMediaTypeCopperCable;
                }
                i++;
            }
            if (((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_25G_KR1_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_25G_KR1_MASK) ||
                ((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_25G_CR1_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_25G_CR1_MASK)) {
                if (fec_cl74 || fec_cl91) {
                    if (fec_cl74) {
                        PHYMOD_IF_ERR_RETURN(
                          _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                                                25000, 1, phymod_fec_CL74, pause, phymod_channel_long, phymod_AN_MODE_CL73BAM));
                        if ((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_25G_CR1_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_25G_CR1_MASK) {
                            autoneg_abilities->autoneg_abilities[i].medium = phymodFirmwareMediaTypeCopperCable;
                        }
                        i++;
                    }
            ...
            if (((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_20G_KR1_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_20G_KR1_MASK) ||
                ((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_20G_CR1_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_20G_CR1_MASK)) {
                if (fec_cl74) {
                    PHYMOD_IF_ERR_RETURN(
                      _tscomod_an_ability_construct(&(autoneg_abilities->autoneg_abilities[i]),
                ...
                if ((up1_1 >> TSCOMOD_AN_UF_PAGE1_BAM_20G_CR1_OFFSET) & TSCOMOD_AN_UF_PAGE1_BAM_20G_CR1_MASK) {
                    autoneg_abilities->autoneg_abilities[i].medium = phymodFirmwareMediaTypeCopperCable;
                }
                i++;
            }
...
int tscomod_pll_to_vco_get(tscomod_refclk_t ref_clock, uint32_t pll, uint32_t *vco)
{
    if (ref_clock == TSCOMOD_REF_CLK_312P5MHZ) {
        switch  (pll) {
            ...
            case phymod_TSCBH_PLL_DIVNONE:
                 *vco = TSCOMOD_VCO_NONE;
                 break;
            default:
                 *vco = TSCOMOD_VCO_INVALID;
                 break;
        }
    } else {
...
int tscomod_plldiv_lkup_get(PHYMOD_ST* pc, int mapped_speed_id, tscomod_refclk_t refclk,  uint32_t *plldiv)
{
    if (refclk == TSCOMOD_REF_CLK_312P5MHZ) {
        *plldiv = tscomod_sc_pmd_entry[mapped_speed_id].pll_mode;
    }

    return PHYMOD_E_NONE;
}

int tscomod_pmd_rx_lock_override_enable(PHYMOD_ST* pc, uint32_t enable)
{
...
int tscomod_polling_for_sc_done(PHYMOD_ST* pc)
{
    int cnt;
...
int tscomod_read_sc_done(PHYMOD_ST* pc)
{
    SC_X2_STSr_t SC_X2_CONTROL_SC_X2_CONTROL_STATUSr_reg;
...
int tscomod_read_sc_fsm_status(PHYMOD_ST* pc)
{
...
int tscomod_pcs_clk_blk_en(const PHYMOD_ST* pc, uint32_t en)
{
...
/* this function is to try see if the current port is 8 lane or not */
STATIC
int _tscomod_4lane_port_get(PHYMOD_ST *pc, int *is_4lane_port)
{
    PHYMOD_ST pc_copy;
    PMD_X1_PMD_X1_TX_MUX_SELSr_t PMD_X1_PMD_X1_TX_MUX_SELS_reg;
    ...
    *is_4lane_port = 0;
    ...
    /* first set the lane mask properly */
    pc_copy.lane_mask = 0x1;
    PMD_X1_PMD_X1_TX_MUX_SELSr_CLR(PMD_X1_PMD_X1_TX_MUX_SELS_reg);
    PHYMOD_IF_ERR_RETURN
        (READ_PMD_X1_PMD_X1_TX_MUX_SELSr(&pc_copy, &PMD_X1_PMD_X1_TX_MUX_SELS_reg));

    if (PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL0_SELf_GET(PMD_X1_PMD_X1_TX_MUX_SELS_reg)) {
        *is_4lane_port = 1;
    } else if (PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL1_SELf_GET(PMD_X1_PMD_X1_TX_MUX_SELS_reg)) {
        *is_4lane_port = 1;
    } else if (PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL2_SELf_GET(PMD_X1_PMD_X1_TX_MUX_SELS_reg)) {
        *is_4lane_port = 1;
    } else if (PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL3_SELf_GET(PMD_X1_PMD_X1_TX_MUX_SELS_reg)) {
        *is_4lane_port = 1;
    } else if (PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL4_SELf_GET(PMD_X1_PMD_X1_TX_MUX_SELS_reg)) {
        *is_4lane_port = 1;
    } else if (PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL5_SELf_GET(PMD_X1_PMD_X1_TX_MUX_SELS_reg)) {
        *is_4lane_port = 1;
    } else if (PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL6_SELf_GET(PMD_X1_PMD_X1_TX_MUX_SELS_reg)) {
        *is_4lane_port = 1;
    } else if (PMD_X1_PMD_X1_TX_MUX_SELSr_TX_PL7_SELf_GET(PMD_X1_PMD_X1_TX_MUX_SELS_reg)) {
        *is_4lane_port = 1;
    }
    return PHYMOD_E_NONE;
}

int tscomod_port_start_lane_get(PHYMOD_ST *pc, int *port_starting_lane, int *port_num_lane)
{
    int start_lane, num_of_lane;
    PHYMOD_ST pc_copy;
    ...
    if (num_of_lane > 1) {
        ...
        *port_num_lane = num_of_lane;
        ...
        if (port_mode_sel_reg == TSCOMOD_PORT_MODE_MULTI_MPP) {
            if (start_lane > 3) {
                *port_starting_lane = 3;
                *port_num_lane = 4;
            } else {
            /* need to check if 4 lane or not */
                int is_4_lane_port = 0;
                PHYMOD_IF_ERR_RETURN
                    (_tscomod_4lane_port_get(&pc_copy, &is_4_lane_port));
                if (is_4_lane_port) {
            ...
            if (start_lane  < 4) {
                switch (port_mode_sel_reg) {
                    case  TSCOMOD_PORT_MODE_SINGLE:
                        *port_starting_lane = 0;
                        *port_num_lane = 4;
                        break;
                    case  TSCOMOD_PORT_MODE_DUAL:
                        if (start_lane < 2) {
                            *port_starting_lane = 0;
                            *port_num_lane = 2;
                        } else {
...
int tscomod_fec_bypass_indication_set(PHYMOD_ST* pc, uint32_t rsfec_bypass_indication)
{
...
int tscomod_fec_bypass_indication_get(PHYMOD_ST *pc, uint32_t *rsfec_bypass_indication)
{
    uint32_t val = 0;
    RX_X2_RS_FEC_RX_CTL0r_t   RX_X2_RS_FEC_RX_CTL0r_reg;
...
int tscomod_vco_to_pll_lkup(uint32_t vco, tscomod_refclk_t refclk, uint32_t* pll_div)
{
    switch(refclk) {
        case TSCOMOD_REF_CLK_312P5MHZ:
            if (vco == TSCOMOD_VCO_41G) {
                *pll_div = TSCOMOD_PLL_MODE_DIV_132;
            } else if (vco == TSCOMOD_VCO_51G) {
                *pll_div = TSCOMOD_PLL_MODE_DIV_165;
            } else {
                *pll_div = TSCOMOD_PLL_MODE_DIV_170;
            }
            break;
        default:
            break;
...
int tscomod_fec_cobra_enable(PHYMOD_ST *pc, uint32_t enable)
{
...
int tscomod_pcs_ts_config(PHYMOD_ST *pc, int sfd, int rpm)
{
    RX_X2_RX_TS_CTLr_t RX_X2_RX_TS_CTLr_reg;
...
int tscomod_pcs_rx_ts_en(PHYMOD_ST* pc, uint32_t en)
{
    RX_X2_RX_TS_CTLr_t RX_X2_RX_TS_CTLr_reg;
...
@returns PHYMOD_E_NONE if no errors. PHYMOD_EERROR else.
@details Enables ts in rx path */
int tscomod_pcs_rx_deskew_en(PHYMOD_ST *pc, int en)
{
    RX_X2_RX_TS_CTLr_t RX_X2_RX_TS_CTLr_reg;
...
float _tscomod_pcs_vco_to_clk_period(uint32_t vco, int os_mode, int pam4)
{
    float clk_period = 0, vco_val = 0;

    switch (vco) {
        case TSCOMOD_VCO_41G:
            vco_val = 41.2500;
            break;
        case TSCOMOD_VCO_51G:
            vco_val = 51.625;
            break;
        case TSCOMOD_VCO_53G:
            vco_val = 53.125;
            break;
        default:
            break;
...
int tscomod_set_pmd_timer_offset(PHYMOD_ST *pc, float ts_clk_period)
{
...
/***********************************************************************************************************************/
#ifdef DV
uint32_t tscomod_pcs_calc_ui_value(tsc_speed_id_e speed_id, int num_pcs_lanes)
{
    uint32_t ui_value;
    float bit_ui_val;
...
uint32_t _tscomod_pcs_vco_to_ui(uint32_t vco, int os_mode, int pam4)
{
    uint32_t ui_value;
    ui_value = 0;
    ...
    switch (vco) {
        case TSCOMOD_VCO_41G:
             ...
             */
                ui_value = TSCOMOD_UI_20G_NRZ_OSX2;
            } else {
                /* 1/20.625 = 0.04848..
                 * make 32 bit 0.04848.. * 2^32 = 208240838 = 0xc6980c6
                 */
                ui_value = TSCOMOD_UI_20G_NRZ_OSX1;
            }
            break;
        case TSCOMOD_VCO_51G:
            if (pam4) {
...
#ifdef SERDES_API_FLOATING_POINT
float _tscomod_pcs_calc_bit_ui_value(uint32_t vco, int os_mode, int pam4)
{
    uint32_t ui_hi_lo;
    float bit_ui_val, bit_ui_val1, bit_ui_val2;

    ui_hi_lo = _tscomod_pcs_vco_to_ui(vco, os_mode, pam4);
...
float _tscomod_pcs_vco_to_tx_clk_period(uint32_t vco, int os_mode, int pam4)
{
    float clk_period;
    float bit_ui_val;
    clk_period = 0.0;

    bit_ui_val = _tscomod_pcs_calc_bit_ui_value(vco, os_mode, pam4);
...
#if 0
int tscomod_set_fclk_period(PHYMOD_ST *pc, uint32_t vco, int clk4sync_div)
{
...
@returns PHYMOD_E_NONE if no errors. PHYMOD_EERROR else.
@details Enables ts in rx path */
int tscomod_pcs_set_1588_ui(PHYMOD_ST *pc, uint32_t vco, int os_mode, int pam4)
{
    PMD_X2_UI_VALUE_HIr_t PMD_X2_UI_VALUE_HIr_reg;
    PMD_X2_UI_VALUE_LOr_t PMD_X2_UI_VALUE_LOr_reg;
    int ui_value_hi, ui_value_lo;

    ui_value_hi = _tscomod_pcs_vco_to_ui(vco, os_mode, pam4) >> 16;
    ui_value_lo = (_tscomod_pcs_vco_to_ui(vco, os_mode, pam4) & 0xffff) >> 9;

    PMD_X2_UI_VALUE_HIr_CLR(PMD_X2_UI_VALUE_HIr_reg);
    PMD_X2_UI_VALUE_LOr_CLR(PMD_X2_UI_VALUE_LOr_reg);

    PMD_X2_UI_VALUE_HIr_UI_FRAC_M1_TO_M16f_SET(PMD_X2_UI_VALUE_HIr_reg, ui_value_hi);
    PMD_X2_UI_VALUE_LOr_UI_FRAC_M17_TO_M23f_SET(PMD_X2_UI_VALUE_LOr_reg, ui_value_lo);

    PHYMOD_IF_ERR_RETURN(MODIFY_PMD_X2_UI_VALUE_HIr(pc, PMD_X2_UI_VALUE_HIr_reg));
    PHYMOD_IF_ERR_RETURN(MODIFY_PMD_X2_UI_VALUE_LOr(pc, PMD_X2_UI_VALUE_LOr_reg));
...
@returns PHYMOD_E_NONE if no errors. PHYMOD_EERROR else.
@details Enables ts in rx path */
int _tscomod_calc_pmd_latency(uint32_t vco, uint32_t os_mode, int pam4, int tx_or_rx)
{
 ...
 */
    if (pam4) {
        /* PAM4 */
       if (tx_or_rx == 1) {
            rx_latency = ((uint32_t)((bit_ui_val * ((TSCOMOD_PMD_RX_DP_LATENCY_PAM4 * 2) + 20)) * 16));
        } else {
            tx_latency = ((uint32_t)((bit_ui_val * ((TSCOMOD_PMD_TX_DP_LATENCY_PAM4 * 2) - 20)) * 1024));
        }
    } else {
        /* NRZ */
        if (tx_or_rx == 1) {
            rx_latency = ((uint32_t)((bit_ui_val * (TSCOMOD_PMD_RX_DP_LATENCY_NRZ + 20)) * 16));
        } else {
            tx_latency = ((uint32_t)((bit_ui_val * (TSCOMOD_PMD_TX_DP_LATENCY_NRZ - 20)) * 1024));  /* delay number from Blackhawk_V1.0 User Spec.pdf */
        }
    }
#endif
...
int tscomod_tx_pmd_latency_get(PHYMOD_ST *pc, int *tx_latency)
{
...
@returns PHYMOD_E_NONE if no errors. PHYMOD_EERROR else.
@details Enables ts in rx path */
int tscomod_tx_pmd_latency_set(PHYMOD_ST *pc, uint32_t vco, int os_mode, int pam4)
{
    int tx_latency;
    PMD_X4_TX_FIXED_LATENCYr_t PMD_X4_TX_FIXED_LATENCYr_reg;

    tx_latency = _tscomod_calc_pmd_latency(vco, os_mode, pam4, 0);
...
int _tscomod_update_tx_pmd_latency(PHYMOD_ST *pc, uint32_t latency_adj, int normalize_to_latest)
{
...
@returns PHYMOD_E_NONE if no errors. PHYMOD_EERROR else.
@details Enables ts in rx path */
int tscomod_pcs_set_tx_lane_skew_capture(PHYMOD_ST *pc, int tx_skew_en)
{
...
@returns PHYMOD_E_NONE if no errors. PHYMOD_EERROR else.
@details Enables ts in rx path */
int tscomod_pcs_measure_tx_lane_skew(PHYMOD_ST *pc, uint32_t vco, int os_mode, int pam4, int normalize_to_latest, int *tx_max_skew)
{
#ifdef SERDES_API_FLOATING_POINT
    int32_t latency_adj;
    float bit_ui_val;
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (_tscomod_update_tx_pmd_latency(&phy_copy, latency_adj, normalize_to_latest));
    }
#endif
    return PHYMOD_E_NONE;
}
...
@returns PHYMOD_E_NONE if no errors. PHYMOD_EERROR else.
@details Enables ts in rx path */
int tscomod_measure_n_normalize_tx_lane_skew(PHYMOD_ST *pc, uint32_t vco, int os_mode, int is_pam4, int fixed_lane, int *tx_max_skew)
{
#ifdef SERDES_API_FLOATING_POINT
    int32_t latency_adj;
    float bit_ui_val;
    ...
    bit_ui_val = _tscomod_pcs_calc_bit_ui_value(vco, os_mode, is_pam4);
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        phy_copy.lane_mask = 0x1 << curr_lane;
        PHYMOD_IF_ERR_RETURN
            (_tscomod_update_tx_pmd_latency(&phy_copy, latency_adj, 0));
    }
#endif
    return PHYMOD_E_NONE;
}

int _tscomod_virtual_lane_count_get(int bit_mux_mode, int num_lane, int *virtual_lanes, int *num_psll_per_phyl)
{
    switch (bit_mux_mode) {
        case 0:
...
int tscomod_mod_rx_1588_tbl_val(PHYMOD_ST *pc, int bit_mux_mode, uint32_t vco,
                                    ...
                                    uint32_t *rx_dsl_sel, uint32_t *rx_psll_sel)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (_tscomod_virtual_lane_count_get(bit_mux_mode, num_lane, &virtual_lanes, &num_psll_per_phyl));
...
int tscomod_chk_rx_ts_deskew_valid(PHYMOD_ST *pc, int bit_mux_mode, int *rx_ts_deskew_valid)
{
...
int tscomod_rsfec_symbol_error_counter_get(PHYMOD_ST* pc,
                                          int bit_mux_mode,
                                          int max_count,
                                          int* actual_count,
...
int tscomod_rs_fec_hi_ser_get(PHYMOD_ST* pc, uint32_t* hi_ser_lh, uint32_t* hi_ser_live)
{
...
int tscomod_intr_status_get(PHYMOD_ST* pc, tscomod_intr_status_t* intr_status)
{
 ...
 */
int tscomod_pmd_osmode_set(PHYMOD_ST* pc, int mapped_speed_id, tscomod_refclk_t refclk)
{
    ...
    * 0x11= OS_MODE_33 */
    ...
    if (refclk == TSCOMOD_REF_CLK_312P5MHZ) {
        os_mode =  tscomod_sc_pmd_entry[mapped_speed_id].t_pma_os_mode;
    }
...
int tscomod_tx_ts_info_unpack_tx_ts_tbl_entry(uint32_t *tx_ts_tbl_entry, phymod_ts_fifo_status_t *tx_ts_info)
{
    int valid;
...
int tscomod_port_enable_set(PHYMOD_ST* pc, int enable)
{
    SC_X2_SW_SPARE1r_t SC_X2_SW_SPARE1r_reg;
    uint16_t temp_reg_value;
    ...
    temp_reg_value = SC_X2_SW_SPARE1r_GET(SC_X2_SW_SPARE1r_reg);
    /* first clear the lowest bit */
    temp_reg_value  &= ~TSCOMOD_PORT_ENABLE_MASK;
    temp_reg_value |= (enable & 0x1) << TSCOMOD_PORT_ENABLE_BIT_SHIFT;
    SC_X2_SW_SPARE1r_SET(SC_X2_SW_SPARE1r_reg, temp_reg_value);
    PHYMOD_IF_ERR_RETURN(WRITE_SC_X2_SW_SPARE1r(pc, SC_X2_SW_SPARE1r_reg));
...
int tscomod_port_enable_get(PHYMOD_ST* pc, int* enable)
{
    SC_X2_SW_SPARE1r_t SC_X2_SW_SPARE1r_reg;
    uint16_t temp_reg_value;
    ...
    PHYMOD_IF_ERR_RETURN(READ_SC_X2_SW_SPARE1r(pc, &SC_X2_SW_SPARE1r_reg));
    temp_reg_value = SC_X2_SW_SPARE1r_GET(SC_X2_SW_SPARE1r_reg);
    *enable = (temp_reg_value & TSCOMOD_PORT_ENABLE_MASK) >> TSCOMOD_PORT_ENABLE_BIT_SHIFT;

    return PHYMOD_E_NONE;
}

/* this function  is to store port autoneg enable info */
int tscomod_port_an_mode_enable_set(PHYMOD_ST* pc, int enable)
{
    SC_X2_SW_SPARE1r_t SC_X2_SW_SPARE1r_reg;
    uint16_t temp_reg_value;
    ...
    temp_reg_value = SC_X2_SW_SPARE1r_GET(SC_X2_SW_SPARE1r_reg);
    /* first clear the lowest bit */
    temp_reg_value  &= ~TSCOMOD_PORT_AN_ENABLE_MASK;
    temp_reg_value |= (enable & 0x1) << TSCOMOD_PORT_AN_ENABLE_BIT_SHIFT;
    SC_X2_SW_SPARE1r_SET(SC_X2_SW_SPARE1r_reg, temp_reg_value);
    PHYMOD_IF_ERR_RETURN(WRITE_SC_X2_SW_SPARE1r(pc, SC_X2_SW_SPARE1r_reg));
...
int tscomod_port_an_mode_enable_get(PHYMOD_ST* pc, int* enable)
{
    SC_X2_SW_SPARE1r_t SC_X2_SW_SPARE1r_reg;
    uint16_t temp_reg_value;
    ...
    PHYMOD_IF_ERR_RETURN(READ_SC_X2_SW_SPARE1r(pc, &SC_X2_SW_SPARE1r_reg));
    temp_reg_value = SC_X2_SW_SPARE1r_GET(SC_X2_SW_SPARE1r_reg);
    *enable = (temp_reg_value & TSCOMOD_PORT_AN_ENABLE_MASK) >> TSCOMOD_PORT_AN_ENABLE_BIT_SHIFT ;

    return PHYMOD_E_NONE;
}

int tscomod_port_cl73_enable_set(PHYMOD_ST* pc, int enable)
{
    AN_X2_CL73_CFGr_t      AN_X2_CL73_CFGr_reg;
...
int tscomod_port_cl73_enable_get(PHYMOD_ST* pc, int* enable)
{
    AN_X2_CL73_CFGr_t      AN_X2_CL73_CFGr_reg;
...
/* the functionbelow will return pcs register num of copy and
 * valid return will be 1, 2 or 4
 */
int tscomod_pcs_reg_num_copy_get(uint32_t reg_addr, int *num_copy)
{
    uint32_t temp_reg;

    temp_reg = reg_addr & 0xffff;
    /* first check if valid PCS reg */
    if (temp_reg < TSCOMOD_PCS_REG_START_ADDR) {
        *num_copy = 0;
        return PHYMOD_E_UNAVAIL;
    } else if (temp_reg < TSCOMOD_PCS_ONE_COPY_REG) {
        *num_copy = 1;
    } else if ((temp_reg > TSCOMOD_PCS_FOUR_COPY_REG_BLOCK1) &&
               (temp_reg < TSCOMOD_PCS_FOUR_COPY_REG_BLOCK1 + 0xf)) {
        *num_copy = 4;
    } else if ((temp_reg > TSCOMOD_PCS_FOUR_COPY_REG_BLOCK2) &&
               (temp_reg < TSCOMOD_PCS_FOUR_COPY_REG_BLOCK2 + 0xf)) {
        *num_copy = 4;
    } else if ((temp_reg > TSCOMOD_PCS_FOUR_COPY_REG_BLOCK3) &&
               (temp_reg < TSCOMOD_PCS_FOUR_COPY_REG_BLOCK3 + 0xf)) {
        *num_copy = 4;
    } else {
        *num_copy = 2;
    }
...
int tscomod_pmd_override_enable_set(PHYMOD_ST* pc,
