/*! \file bcm56996_a0_phy_fw_load.c
 *
 * BCM56996_A0 PHY firmware loader.
 */
/*
...
#define BCM56996_CDPORT_TSC_UCMEM_DATA_SIZE 122880
...
static int
tsco_firmware_helper(const phymod_core_access_t *ca,
                      uint32_t fw_size, const uint8_t *fw_data)
{
    bcmpc_phy_access_data_t *phy_data = PHYMOD_ACC_USER_ACC(&ca->access);
    int unit = phy_data->unit;
    ...
    ucmem_idx_max = BCM56996_CDPORT_TSC_UCMEM_DATA_SIZE / FW_ALIGN_BYTES - 1;
...
static int
pm_info_update(int unit, int pm_id, bcmpc_topo_t *pm_info, void *cookie)
{
    ...
    if ((phy_type == phymodDispatchTypeTscbh) ||
        (phy_type == phymodDispatchTypeTsce7)) {
        /* Firmware is download on the PM. */
...
static int
pm_tsco_firmware_load(int unit, int *last_bcast_port, bcmdrd_pbmp_t *pbmp)
{
    ...
    if (fw_cfg.fw_load == PM_SERDES_FW_LOAD_SLOW) {
        BCMDRD_PBMP_ITER(pbmp[0], port) {
            ...
            SHR_IF_ERR_EXIT
                (bcmpc_phymod_core_access_t_init(unit, port, phy_data, ca));
            sal_memset(cic, 0, sizeof(*cic));
            cic->firmware_loader = tsco_firmware_helper;
            SHR_IF_ERR_EXIT
                (bcmpc_phymod_fw_load_config_set(unit, &fw_cfg, cic));
            PHYMOD_CORE_INIT_F_EXECUTE_PASS1_SET(cic);
        ...
        for (idx = 0; idx < CDPORT_NUM_BCAST; idx++) {
            if (last_bcast_port[idx] > 0 ) {
                ...
                bcast_port = CDPORT_BCAST_PORT_U0(idx);
                BCMPC_PHY_ACCESS_DATA_SET(phy_data, unit, bcast_port);
                SHR_IF_ERR_EXIT
                    (bcmpc_phymod_core_access_t_init(unit,
                                                     phy_port, phy_data, ca));
...
cdport_last_bcast_port_get(bcmdrd_pbmp_t *pbmp, int *last_bcast_port)
{
    ...
    BCMDRD_PBMP_ITER(pbmp[0], port) {
        /* Get the sbus ring number that the port belongs to.
         * only U0 broadcast ID will be required */
        bcast_domain = CDPORT_BCAST_DOMAIN(port);
        if (PORT_IS_U0(port)) {
            if (bcast_domain & 0x1) {
                                 ...
                                 phymod_firmware_loader_f *fw_loader)
{
    ...
    int last_bcast_port[CDPORT_NUM_BCAST] = {-1, -1, -1, -1};
    ...
    if (!fw_loader_req) {
        ...
        /* Get port bitmap for each serdes core. */
        SHR_IF_ERR_EXIT
            (bcmpc_topo_traverse(unit, pm_port_pbmp_get, tsc_pbmp));
        /* CDPORT broadcast is already enabled by BCMBD. */
        cdport_last_bcast_port_get(tsc_pbmp, last_bcast_port);
        /* Bcast Download firmware on TSCO. */
        SHR_IF_ERR_EXIT
            (pm_tsco_firmware_load(unit, last_bcast_port, tsc_pbmp));
        SHR_IF_ERR_EXIT
        ...
        phy_type = pc->phy_drv_get(unit, port);
        if (phy_type == phymodDispatchTypeTsco) {
            *fw_loader = tsco_firmware_helper;
        } else if (phy_type == phymodDispatchTypeTsce7) {
