...
static err_code_t _osprey7_v2l8p1_display_eye_scan_ignored_lane(srds_access_t *sa__) {
    EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
    return (ERR_CODE_NONE);
}
...
err_code_t osprey7_v2l8p1_display_eye_scan(srds_access_t *sa__) {
    ...
    err_code_t err_code_setup = ERR_CODE_NONE;

    err_code_setup = osprey7_v2l8p1_INTERNAL_eye_scan_setup(sa__, 0, &y_max, &y_step);
    if((err_code_setup == ERR_CODE_RX_TUNING_NOT_DONE) || ((enum srds_diag_failcodes)err_code_setup == ERR_CODE_DIAG_SCAN_NO_PMD_LOCK)) {
        return ERR_CODE_NONE;
    }
    ...
    EFUN(osprey7_v2l8p1_display_eye_scan_header(sa__,1));
    ...
    for (y = y_max;y>=-y_max;y=(int16_t)(y-y_step)) {
        ...
        if(enable_read){
            err_code_t err_code = osprey7_v2l8p1_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (err_code) {
                EFUN(osprey7_v2l8p1_meas_eye_scan_done(sa__));
            }
        }
        EFUN(osprey7_v2l8p1_display_eye_scan_stripe(sa__, y,&stripe[buffer_idx]));
        EFUN_PRINTF(("\n"));
    }
    EFUN(osprey7_v2l8p1_display_eye_scan_footer(sa__,1));
    /* stop acquisition */
    EFUN(osprey7_v2l8p1_meas_eye_scan_done(sa__));
...
err_code_t osprey7_v2l8p1_display_all_eye_scan(srds_access_t *sa__) {
    ...
    for(lane=0;lane < num_lanes;lane++) {
        ...
        err_code = osprey7_v2l8p1_INTERNAL_eye_scan_setup(sa__, 0, &i_max[lane], &i_step[lane]);
        if(err_code) {
            uint8_t core;
            ...
            lane_mask |= (uint16_t)(1<<lane);
        }
        ESTM(eye_slice[lane] = ((rdv_usr_diag_status() & DIAG_EYE_SCAN_RANGE_MASK) == DIAG_EYE_SCAN_SLICE));
        i[lane] = i_max[lane];
    }

    EFUN(osprey7_v2l8p1_display_eye_scan_header(sa__, num_lanes));
        ...
        for(lane=0; lane < num_lanes; lane++) {
            if(lane_mask & (1<<lane)) {
                if(i[lane] >= -i_max[lane]) {
                    ...
                    if(enable_read) {
                        EFUN(osprey7_v2l8p1_read_eye_scan_stripe(sa__, &stripe[0], &status));
                    }

                    EFUN(osprey7_v2l8p1_display_eye_scan_stripe(sa__, i[lane],&stripe[buffer_idx]));
                    ...
                    if(eye_slice[lane]) {
                        ...
                        EFUN(_osprey7_v2l8p1_display_eye_scan_ignored_lane(sa__));
                    }
                }
            }
            else {
    ...
    EFUN(osprey7_v2l8p1_display_eye_scan_footer(sa__, num_lanes));
    for(lane=0;lane < num_lanes;lane++) {
        if(lane_mask & (1<<lane)) {
            EFUN(osprey7_v2l8p1_acc_set_lane(sa__, lane));
            EFUN(osprey7_v2l8p1_meas_eye_scan_done(sa__));
...
err_code_t osprey7_v2l8p1_display_intrusive_eye_scan(srds_access_t *sa__) {
    ...
    Exit_Status = osprey7_v2l8p1_INTERNAL_get_intrusive_eye_scan(sa__, data, &y_max, &y_step, &lane_mask, 1);
    ...
    CFUN(osprey7_v2l8p1_display_eye_scan_header(sa__,1));
    for (y = y_max;y>=-y_max;y=(int16_t)(y-y_step)) {
        ...
        CFUN(osprey7_v2l8p1_display_eye_scan_stripe(sa__, y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    CFUN(osprey7_v2l8p1_display_eye_scan_footer(sa__,1));
...
err_code_t osprey7_v2l8p1_display_all_intrusive_eye_scan(srds_access_t *sa__) {
    ...
    EFUN(osprey7_v2l8p1_INTERNAL_get_intrusive_eye_scan(sa__, data, i_max, i_step, &lane_mask, num_lanes));

    if(lane_mask) {
        CFUN(osprey7_v2l8p1_display_eye_scan_header(sa__, num_lanes));
    }
    else {
        goto Exit;
    }
        ...
        for(lane=0; lane < num_lanes; lane++) {
            if(lane_mask & (1<<lane)) {
                if(i[lane] >= -i_max[lane]) {
                    ...
                    err = osprey7_v2l8p1_display_eye_scan_stripe(sa__, i[lane],&stripe[0]);
                    if (err) {
                        lane_mask &=  ~(1<<lane);
                    ...
                    CFUN(_osprey7_v2l8p1_display_eye_scan_ignored_lane(sa__));
                }
            }
            else {
                CFUN(_osprey7_v2l8p1_display_eye_scan_ignored_lane(sa__));
    ...
    CFUN(osprey7_v2l8p1_display_eye_scan_footer(sa__, num_lanes));
...
Exit:
    ...
    if(Exit_Status != ERR_CODE_NONE) {
        EFUN_PRINTF(("Unrecoverable error occurred during osprey7_v2l8p1_display_all_intrusive_eye_scan():\n."));
        return osprey7_v2l8p1_error_report(sa__, Exit_Status);
    }
    return(first_err);
}
...
err_code_t osprey7_v2l8p1_display_eye_scan_stripe(srds_access_t *sa__, int16_t y,uint32_t *buffer) {
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p1_log_api_event(srds_access_t *sa__, uint8_t event_id, uint8_t data1, uint8_t data2) {
    INIT_SRDS_ERR_CODE
    ...
    if(osprey7_v2l8p1_info_ptr->info_table_version > 0x38) {
        ...
        EFUN(osprey7_v2l8p1_pmd_uc_cmd_with_data(sa__, CMD_API_EVENT, event_id, data_new, GRACEFUL_STOP_TIME));
    }
    return(ERR_CODE_NONE);
}
err_code_t osprey7_v2l8p1_display_eye_scan_header(srds_access_t *sa__, uint8_t i) {
    INIT_SRDS_ERR_CODE
    uint8_t x;
    ...
    for(x=1;x<=i;x++) {
        if((diag_sts[x-1]&DIAG_EYE_SCAN_RANGE_MASK) == DIAG_EYE_SCAN_SLICE) {
            EFUN_PRINTF(("         : 0"));
        } else if((diag_sts[x-1]&DIAG_EYE_SCAN_RANGE_MASK) == DIAG_EYE_SCAN_INTR) {
...
err_code_t osprey7_v2l8p1_display_eye_scan_footer(srds_access_t *sa__, uint8_t i) {
    INIT_SRDS_ERR_CODE
    uint8_t x;
    uint8_t stepsize_var[NUM_LANES_MAX];
    uint8_t lane, num_lanes;
 ...
 */
    ...
    do {
        if(heye == 1) {
            ...
            if(intrusive) {
                slicer = LMS_SLICER;
                EFUN(osprey7_v2l8p1_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], 0, slicer, &nlmv_val));
                margins[i] = direction * nlmv_val;
            } else {
                slicer = LMS_SLICER;
                EFUN(osprey7_v2l8p1_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], range250, slicer, &nlmv_val));
                margins[i] = direction * nlmv_val;
            }
#else
            margins[i] = info_out->margins[i];
...
err_code_t osprey7_v2l8p1_display_lane_state_hdr(srds_access_t *sa__) {
  EFUN_PRINTF(("LN (RX  , CDRxN , UC_CFG,   UC_STS,  RST, STP) "));
  EFUN_PRINTF(("SD LCK RXPPM "));
  EFUN_PRINTF(("CLK90 "));
  EFUN_PRINTF(("PF(M,L,H)  "));
  EFUN_PRINTF(("BW(0,1, 2,3,4,5)  "));
  EFUN_PRINTF(("VGA "));
  EFUN_PRINTF(("DCO P1mV "));
  EFUN_PRINTF(("       TP(0,1a,1b,2,3,4,5,6)       "));
  EFUN_PRINTF(("RXFFE(g,1,2)  "));
  EFUN_PRINTF(("     DFE(1,2,3,4,5,6)       TXPPM    "));
...
err_code_t osprey7_v2l8p1_display_lane_state_legend(srds_access_t *sa__) {
  ...
  EFUN_PRINTF(("PF(M,L,H): Peaking Filter Main (0..15), Low Freq (0..7/15), and High Freq (0..15)\n"));
  ...
  EFUN_PRINTF(("         2 - usr_status_eq_debug2\n"));
  EFUN_PRINTF(("         3 - usr_status_eq_debug1\n"));
  EFUN_PRINTF(("         4 - tp_metric_prec1\n"));
  EFUN_PRINTF(("         5 - usr_status_tp_metric_2\n"));
  EFUN_PRINTF(("         6 - usr_status_tp_metric_3\n"));
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    if(ber_scan_mode & DIAG_BER_HORZ) {
        ...
        slicer = LMS_SLICER;
        EFUN(osprey7_v2l8p1_INTERNAL_ladder_setting_to_mV(sa__, offset_start, range250, slicer, &nlmv_val));
    ...
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(osprey7_v2l8p1_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, 200));
        ESTM(dataword = rd_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
        time_byte = (uint8_t)(dataword>>8);          /* MSB is time byte */
        prbs_multi = (uint8_t)dataword & 0x0F;       /* split nibbles */
        time_multi = (uint8_t)dataword>>4;
        /* Read 1 bytes of data */
        EFUN(osprey7_v2l8p1_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, 200));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        ...
        if(DIAG_VERBOSE < 5) {
            ...
            if(ber_scan_mode & DIAG_BER_HORZ) {
                ...
                slicer = LMS_SLICER;
                EFUN(osprey7_v2l8p1_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)(SRDS_ABS(offset_start)-i), range250, slicer, &nlmv_val));
    ...
    EFUN_PRINTF(("\n"));
    EFUN(osprey7_v2l8p1_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,2000));
...
err_code_t osprey7_v2l8p1_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    ESTM(rx_lock = rd_pmd_rx_lock());
    {
        ...
        micro_stop = osprey7_v2l8p1_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
...
Exit:
    ...
    if(diag_level & SRDS_DIAG_TIMESTAMP) {
        EFUN(osprey7_v2l8p1_test_uc_timestamp(sa__));
    }
    if(diag_level & SRDS_DIAG_EVENT_SAFE) {
        ...
        for (; micro_num<osprey7_v2l8p1_info_ptr->micro_count; ++micro_num)
        {
            ...
            EFUN(osprey7_v2l8p1_INTERNAL_read_event_log_with_callback(sa__, micro_num, 1, &state, osprey7_v2l8p1_INTERNAL_event_log_dump_callback));
            EFUN(osprey7_v2l8p1_INTERNAL_event_log_dump_callback(sa__, &state, 0, 0));
        }
    }
    if(diag_level & SRDS_DIAG_EVENT) {
        EFUN(osprey7_v2l8p1_read_event_log(sa__));
    }
    /* currently REG_CORE and REG_LANE dump same data. */
    core = osprey7_v2l8p1_acc_get_core(sa__);
    lane = osprey7_v2l8p1_acc_get_lane(sa__);

    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        EFUN(osprey7_v2l8p1_reg_dump(sa__, core, lane, NULL));
    }
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(osprey7_v2l8p1_uc_core_var_dump(sa__, core, NULL));

        {
        uint8_t orig_lane, num_micros, srds_lane;

        EFUN(osprey7_v2l8p1_get_micro_num_uc_cores(sa__, &num_micros));
        orig_lane = osprey7_v2l8p1_acc_get_physical_lane(sa__);
        for (srds_lane = 0; srds_lane < num_micros*2; srds_lane =(uint8_t)(srds_lane + 2)) {
            EFUN(osprey7_v2l8p1_acc_set_physical_lane(sa__, srds_lane));

            EFUN(osprey7_v2l8p1_uc_micro_var_dump(sa__, core, srds_lane, NULL));
        }
        EFUN(osprey7_v2l8p1_acc_set_physical_lane(sa__, orig_lane));
        }
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {

        EFUN(osprey7_v2l8p1_uc_lane_var_dump(sa__, core, lane, NULL));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(osprey7_v2l8p1_display_eye_scan(sa__));
    }
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        EFUN(osprey7_v2l8p1_display_lane_debug_status(sa__));
    }
    ...
    if((diag_level & SRDS_DIAG_PRBS_PROJ)
        ...
        osprey7_v2l8p1_display_aggregate_prbs_error_analyzer_proj(sa__, 5440, SRDS_OSP_100GE, lane_mask_aggregate, timeout_s, diag_mode);
    }

    if(diag_level & SRDS_DIAG_INFO_TABLE) {
        EFUN(osprey7_v2l8p1_INTERNAL_display_info_table(sa__));
    }
    /* re enable micro */
    if (!micro_stop) {
