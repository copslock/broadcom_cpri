...
#ifndef OSPREY7_V2L8P1_API_INTERNAL_H
#define OSPREY7_V2L8P1_API_INTERNAL_H
...
/*---------------------------------------*/
enum osprey7_v2l8p1_prbs_chk_timer_selection_enum {
    USE_HW_TIMERS = 0,
    USE_SW_TIMERS = 1
};
...
typedef struct {
  ...
  /** Tuninig parameter 2 */
  int8_t usr_status_eq_debug2;
  /** Tuninig parameter 5 */
  int8_t usr_status_tp_metric_2;
  /** Tuning parameter 6 */
  int8_t usr_status_tp_metric_3;
  /** BW 0 */
  uint8_t pfmid_capbw;
  /** BW 1 */
  uint8_t pfmid_indbw;
...
typedef struct {
  ...
  /** PLL Divider value.  (Same encoding as enum #osprey7_v2l8p1_pll_div_enum.) */
  uint32_t pll_div;
  /** PLL Lock */
  uint8_t  pll_lock;
  /** PLL Lock Change*/
...
/** Checks to see if the stored ucode_version in osprey7_v2l8p1_info is matching with the current thread ucode_version.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] osprey7_v2l8p1_info_ptr pointer to Info_table to be verified.
...
/** Verify the internal osprey7_v2l8p1_info.
 * NOTE: This function is getting deprecated. Instead, please use the functions
 * osprey7_v2l8p1_INTERNAL_get_osprey7_v2l8p1_info_ptr_with_check and osprey7_v2l8p1_INTERNAL_match_ucode_from_info
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] osprey7_v2l8p1_info_ptr pointer to Info_table to be verified.
 * @return Error code, if generated (returns ERR_CODE_NONE if no errors).
 */
...
/** Return whether the fraction portion of a #osprey7_v2l8p1_pll_div_enum value is nonzero. */
#define SRDS_INTERNAL_IS_PLL_DIV_FRACTIONAL(div_) (((uint32_t)(div_) & 0xFFFFF000UL) != 0)

/** Extract the integer portion of a #osprey7_v2l8p1_pll_div_enum value. */
#define SRDS_INTERNAL_GET_PLL_DIV_INTEGER(div_) ((uint32_t)(((uint32_t)(div_)) & 0x00000FFFUL))

/** Extract the fraction portion of a #osprey7_v2l8p1_pll_div_enum value.
...
/** Compose a value in the same format as a #osprey7_v2l8p1_pll_div_enum value.
 ...
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] refclk Reference clock frequency (enumerated), or OSPREY7_V2L8P1_PLL_REFCLK_UNKNOWN to auto-compute.
 * @param[in,out] refclk_freq_hz Resultant reference clock frequency, in Hz.
 * @param[in,out] srds_div Divider value (enumerated), or OSPREY7_V2L8P1_PLL_DIV_UNKNOWN to auto-compute.
 *             The final divider value will be filled in if OSPREY7_V2L8P1_PLL_DIV_UNKNOWN.
 ...
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] refclk Reference clock frequency (enumerated), or OSPREY7_V2L8P1_PLL_REFCLK_UNKNOWN to auto-compute.
 * @param[in] srds_div Divider value (enumerated), or OSPREY7_V2L8P1_PLL_DIV_UNKNOWN to auto-compute.
 ...
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[out] srds_div Divider value, in the same encoding as enum #osprey7_v2l8p1_pll_div_enum.
 * @return Error Code, if generated (returns ERR_CODE_NONE if no errors).
 */
 ...
 * as part of osprey7_v2l8p1_display_core_state_line().
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
...
} osprey7_v2l8p1_INTERNAL_event_log_dump_state_t;

/** Callback function for dumping event log, intended to be called by osprey7_v2l8p1_INTERNAL_read_event_log_with_callback.
 *  It must also be called once after, with a byte count of 0, to finish printing.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in,out] arg is a osprey7_v2l8p1_INTERNAL_event_log_dump_state_t pointer for holding state.
...
/*-----------------------------*/
/** Read current osprey7_v2l8p1 core status.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[out] istate Current osprey7_v2l8p1 core status read back and populated by the API.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
...
/*-----------------------------*/
/** Read current osprey7_v2l8p1 lane status.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[out] istate Current osprey7_v2l8p1 lane status read back and populated by the API.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
 ...
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
err_code_t osprey7_v2l8p1_INTERNAL_get_osr_mode(srds_access_t *sa__, osprey7_v2l8p1_osr_mode_st *mode);
#endif /* SMALL_FOOTPRINT */
 ...
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[out] sig_det Current osprey7_v2l8p1 lane signal_detect status read back and populated by the API.
 * @param[out] sig_det_chg Current osprey7_v2l8p1 lane signal_detect_change status read back and populated by the API.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
 ...
 * Along with osprey7_v2l8p1_core_clkgate(), all lanes should also be clock gated using osprey7_v2l8p1_lane_clkgate() to complete a Core Clockgate.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 ...
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[out] pmd_lock Current osprey7_v2l8p1 lane pmd_rx_lock status read back and populated by the API.
 * @param[out] pmd_lock_chg Current osprey7_v2l8p1 lane pmd_rx_lock_change status read back and populated by the API.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
 ...
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[out] pll_lock Current osprey7_v2l8p1 lane pll_lock status read back and populated by the API.
 * @param[out] pll_lock_chg Current osprey7_v2l8p1 lane pll_lock_change status read back and populated by the API.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
 ...
 * Define macro CUSTOM_REG_POLLING to replace the default implementation provided in osprey7_v2l8p1_internal.c.
 *
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
...
#define get_endian_offset(addr) (osprey7_v2l8p1_INTERNAL_is_big_endian() ? \
                                (\
...
typedef struct {
    uint8_t *mem_ptr;
} osprey7_v2l8p1_INTERNAL_rdblk_callback_arg_t;

/** Callback function for reading uc RAM memory by osprey7_v2l8p1_INTERNAL_rdblk_uc_generic_ram().
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in,out] arg is a osprey7_v2l8p1_INTERNAL_rdblk_callback_arg_t pointer for holding state.
 ...
 *         indefinitely, until cnt bytes total are read.
 *  In other words, this is the same as osprey7_v2l8p1_INTERNAL_rdblk_uc_generic_ram(), but reading in descending address order.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 ...
 * @param[out] dbgfb_stats pointer of type osprey7_v2l8p1_dbgfb_stats_st.
 * @return Error Code generated by API (ERR_CODE_NONE if no errors).
 */
 ...
 * Note: Call before releasing osprey7_v2l8p1_ln_dp_reset().
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
