...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  osprey7_v2l8p1_prbs.c                                             *
...
err_code_t osprey7_v2l8p1_config_shared_tx_pattern(srds_access_t *sa__, uint8_t patt_length_bits, const char pattern[]) {
    INIT_SRDS_ERR_CODE
  ...
  uint8_t    zero_pad_len    = 0; /* suppress warnings, changed by osprey7_v2l8p1_INTERNAL_calc_patt_gen_mode_sel() */
  uint16_t   patt_gen_wr_val = 0;
  uint8_t    mode_sel        = 0; /* suppress warnings, changed by osprey7_v2l8p1_INTERNAL_calc_patt_gen_mode_sel() */

  EFUN(osprey7_v2l8p1_INTERNAL_calc_patt_gen_mode_sel(sa__, &mode_sel,&zero_pad_len,patt_length_bits));
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    ...
    for (i=2; i < str_len; i++) {
      if (pattern[i] != '_') {
        EFUN(osprey7_v2l8p1_INTERNAL_compute_bin(sa__, pattern[i],bin));
        ...
        if (actual_patt_len > 240) {
          ...
          return (osprey7_v2l8p1_error(sa__, ERR_CODE_CFG_PATT_PATTERN_BIGGER_THAN_MAXLEN));
        }
      }
    }
    ...
    if ((offset_len > 3)  || (actual_patt_len < patt_length_bits)) {
      ...
      return (osprey7_v2l8p1_error(sa__, ERR_CODE_CFG_PATT_LEN_MISMATCH));
    }
    else if (offset_len) {
    ...
    for (i=1; i < str_len; i++) {
      if (pattern[i] != '_') {
        EFUN(osprey7_v2l8p1_INTERNAL_pam4_to_bin(sa__, pattern[i],bin));
        ENULL_STRNCAT(patt_mod, bin, SPC_LEFT(patt_mod) - 1);
    ...
    for (i=0; i < str_len; i++) {
      if ((pattern[i] == '0') || (pattern[i] == '1')) {
        ...
        return (osprey7_v2l8p1_error(sa__, ERR_CODE_CFG_PATT_INVALID_PATTERN));
      }
    }
  ...
  for (i=0; i < 15; i++) {
    ...
    for (j=0; j < 4; j++) {
      ...
      bin[4] = '\0';
      EFUN(osprey7_v2l8p1_INTERNAL_compute_hex(sa__,bin, &hex));
    ...
    switch (i) {
      ...
      default: EFUN_PRINTF(("ERROR: Invalid write to patt_gen_seq register\n"));
               return (osprey7_v2l8p1_error(sa__, ERR_CODE_CFG_PATT_INVALID_SEQ_WRITE));
    }
  }
...
err_code_t osprey7_v2l8p1_tx_shared_patt_gen_en(srds_access_t *sa__, uint8_t enable, uint8_t patt_length_bits) {
    INIT_SRDS_ERR_CODE
  uint8_t zero_pad_len = 0; /* suppress warnings, changed by osprey7_v2l8p1_INTERNAL_calc_patt_gen_mode_sel() */
  uint8_t mode_sel     = 0; /* suppress warnings, changed by osprey7_v2l8p1_INTERNAL_calc_patt_gen_mode_sel() */
  ...
  if (enable) {
    if ((mode_sel < 1) || (mode_sel > 6)) {
      return (osprey7_v2l8p1_error(sa__, ERR_CODE_PATT_GEN_INVALID_MODE_SEL));
    }
...
err_code_t osprey7_v2l8p1_config_tx_jp03b_pattern(srds_access_t *sa__, uint8_t enable) {
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p1_config_tx_linearity_pattern(srds_access_t *sa__, uint8_t enable) {
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p1_config_tx_prbs(srds_access_t *sa__, enum srds_prbs_polynomial_enum prbs_poly_mode, uint8_t prbs_inv) {
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p1_get_tx_prbs_config(srds_access_t *sa__, enum srds_prbs_polynomial_enum *prbs_poly_mode, uint8_t *prbs_inv) {
    INIT_SRDS_ERR_CODE
    uint8_t val;
...
err_code_t osprey7_v2l8p1_tx_prbs_en(srds_access_t *sa__, uint8_t enable) {
    INIT_SRDS_ERR_CODE
    if (enable) {
...
err_code_t osprey7_v2l8p1_get_tx_prbs_en(srds_access_t *sa__, uint8_t *enable) {
    INIT_SRDS_ERR_CODE
    if(!enable) {
...
err_code_t osprey7_v2l8p1_tx_prbs_err_inject(srds_access_t *sa__, uint8_t enable) {
    INIT_SRDS_ERR_CODE
  /* PRBS Error Insert.
...
err_code_t osprey7_v2l8p1_config_rx_prbs(srds_access_t *sa__, enum srds_prbs_polynomial_enum prbs_poly_mode, enum srds_prbs_checker_mode_enum prbs_checker_mode, uint8_t prbs_inv) {
    INIT_SRDS_ERR_CODE
  {
    uint8_t dig_lpbk = 0;
  if(prbs_poly_mode >= PCS_PRBS_7) {
...
/* get PRBS Checker */
err_code_t osprey7_v2l8p1_get_rx_prbs_config(srds_access_t *sa__, enum srds_prbs_polynomial_enum *prbs_poly_mode, enum srds_prbs_checker_mode_enum *prbs_checker_mode, uint8_t *prbs_inv) {
    INIT_SRDS_ERR_CODE
    uint8_t val;
...
err_code_t osprey7_v2l8p1_rx_prbs_en(srds_access_t *sa__, uint8_t enable) {
    INIT_SRDS_ERR_CODE
    if (enable) {
...
err_code_t osprey7_v2l8p1_get_rx_prbs_en(srds_access_t *sa__, uint8_t *enable) {
    INIT_SRDS_ERR_CODE
    if(!enable) {
        return(osprey7_v2l8p1_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
err_code_t osprey7_v2l8p1_prbs_chk_lock_state(srds_access_t *sa__, uint8_t *chk_lock) {
    INIT_SRDS_ERR_CODE
  if(!chk_lock) {
    return(osprey7_v2l8p1_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t osprey7_v2l8p1_prbs_err_count_ll(srds_access_t *sa__, uint32_t *prbs_err_cnt) {
    INIT_SRDS_ERR_CODE
  uint16_t rddata;
...
err_code_t osprey7_v2l8p1_prbs_err_count_state(srds_access_t *sa__, uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
    INIT_SRDS_ERR_CODE
  ...
  if(!prbs_err_cnt || !lock_lost) {
    ...
    EFUN(osprey7_v2l8p1_prbs_err_count_ll(sa__, prbs_err_cnt));
...
err_code_t osprey7_v2l8p1_get_prbs_chk_hw_timer_ctrl(srds_access_t *sa__, struct osprey7_v2l8p1_prbs_chk_hw_timer_ctrl_st * const prbs_chk_hw_timer_ctrl_bak) {
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p1_set_prbs_chk_hw_timer_ctrl(srds_access_t *sa__, struct osprey7_v2l8p1_prbs_chk_hw_timer_ctrl_st const * const prbs_chk_hw_timer_ctrl_bak) {
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p1_config_prbs_chk_hw_timer(srds_access_t *sa__, uint16_t time_ms, uint16_t *time_ms_adjusted) {
    INIT_SRDS_ERR_CODE
    struct osprey7_v2l8p1_prbs_chk_hw_timer_ctrl_st prbs_chk_hw_timer_ctrl;
    ENULL_MEMSET(&prbs_chk_hw_timer_ctrl, 0, sizeof(struct osprey7_v2l8p1_prbs_chk_hw_timer_ctrl_st));

    EFUN(wr_prbs_chk_burst_err_cnt_en(0));

    /* Figure out timer mode and timer count */
    EFUN(osprey7_v2l8p1_INTERNAL_get_prbs_timeout_count_from_time(time_ms, time_ms_adjusted, &prbs_chk_hw_timer_ctrl));
...
err_code_t osprey7_v2l8p1_display_detailed_prbs_state_hdr(srds_access_t *sa__) {
...
static char* osprey7_v2l8p1_e2s_prbs_mode_enum[22] = {
    " PRBS_7",
    " PRBS_9",
    "PRBS_11",
    "PRBS_15",
...
err_code_t osprey7_v2l8p1_display_detailed_prbs_state(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    uint32_t err_cnt = 0;
    uint8_t lock_lost = 0;
    uint8_t enabled;

    ESTM_PRINTF(("  %d ",osprey7_v2l8p1_acc_get_lane(sa__)));
    ...
    if(enabled) {
        ...
        EFUN(osprey7_v2l8p1_get_tx_prbs_config(sa__, &prbs_poly_mode, &prbs_inv));
        if(prbs_poly_mode == PRBS_UNKNOWN) {
            prbs_string = "UNKNOWN";
        }
        else {
            prbs_string =  (char *)osprey7_v2l8p1_e2s_prbs_mode_enum[prbs_poly_mode];
        }
    ...
    if(enabled) {
        ...
        uint8_t prbs_inv = 0;
        const char *prbs_string;
        EFUN(osprey7_v2l8p1_get_rx_prbs_config(sa__, &prbs_poly_mode, &prbs_checker_mode, &prbs_inv));
        if(prbs_poly_mode == PRBS_UNKNOWN) {
            prbs_string = "UNKNOWN";
        }
        else {
    ...
    EFUN(osprey7_v2l8p1_prbs_err_count_state(sa__,&err_cnt,&lock_lost));
...
err_code_t osprey7_v2l8p1_prbs_error_analyzer_reset(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE

    EFUN(wrc_tlb_err_analyze_en(0x0)); /* Disables error counting into the memory */
    EFUN(osprey7_v2l8p1_clear_dac_pattern_memory(sa__));  /* Clear DAC pattern memory */
    return (ERR_CODE_NONE);
}

/* Configure PRBS Error Analyzer */
err_code_t osprey7_v2l8p1_prbs_error_analyzer_config(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    uint8_t  fec_size;
    ...
    if ((err_analyzer->prbs_err_fec_size < OSPREY7_V2L8P1_PRBS_ERR_ANALYZER_FEC_SIZE_MIN) || (err_analyzer->prbs_err_fec_size > OSPREY7_V2L8P1_PRBS_ERR_ANALYZER_FEC_SIZE_MAX)) {
        return (osprey7_v2l8p1_error(sa__, ERR_CODE_INVALID_PRBS_ERR_ANALYZER_FEC_SIZE));
    }

    fec_size = (uint8_t)((err_analyzer->prbs_err_fec_size + OSPREY7_V2L8P1_PRBS_FEC_FRAME_SIZE_ROUND_UP) / OSPREY7_V2L8P1_PRBS_FEC_FRAME_SIZE_MULTIPLIER);

    fec_size_bits = (uint16_t)(fec_size * OSPREY7_V2L8P1_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
    ...
    switch (err_analyzer->prbs_err_aggregate_mode) {
        case SRDS_OSP_100GE:              /* 100G w/ Bit Muxing 4:1 and no Code Word Interleaving 100GE 112G Osprey */
           ...
           err_analyzer->fec_size_frac = OSPREY7_V2L8P1_PRBS_FEC_SIZE_FRAC_1_8;
...
err_code_t osprey7_v2l8p1_prbs_error_analyzer_start(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p1_get_prbs_error_analyzer_config(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    ...
    err_analyzer->prbs_err_fec_size = (uint16_t)(prbs_err_fec_size * OSPREY7_V2L8P1_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
    err_analyzer->fec_size_frac = (uint8_t) prbs_err_fec_size_frac;
    return (ERR_CODE_NONE);
}

/* Display PRBS Error Analyzer Config */
err_code_t osprey7_v2l8p1_display_prbs_error_analyzer_config(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer, uint32_t timeout_s) {
    if(!err_analyzer) {
        return(osprey7_v2l8p1_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    ...
    {
        EFUN_PRINTF(("%u %hu %hhu %u %hhu %hhu %hhu %08x %08x\n",   timeout_s, err_analyzer->prbs_err_fec_size, err_analyzer->fec_size_frac, err_analyzer->prbs_err_aggregate_mode, err_analyzer->lanes_active, err_analyzer->diag_mode, osprey7_v2l8p1_get_lane(sa__), err_analyzer->pp_field2, err_analyzer->pp_field1));
    }
    else
    {
        EFUN_PRINTF(("\n ****************************************************************************** \n"));
...
err_code_t osprey7_v2l8p1_clear_dac_pattern_memory(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    uint8_t i;
    ...
    EFUN(osprey7_v2l8p1_INTERNAL_clk4sync_div2_sequence(sa__, 0));
    ...
    EFUN(osprey7_v2l8p1_INTERNAL_clk4sync_div2_sequence(sa__, 1));

    return (ERR_CODE_NONE);
}

/* Read DAC pattern memory into a 32xN memory */
err_code_t osprey7_v2l8p1_read_dac_pattern_memory(srds_access_t *sa__, uint32_t *dac_mem, uint8_t read_addr, uint8_t num_of_lines) {
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p1_get_prbs_error_analyzer_lane_err_count(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t lane) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(osprey7_v2l8p1_read_dac_pattern_memory(sa__,dacmem,(uint8_t)(4*lane),4));
    ...
    osprey7_v2l8p1_read_dac_pattern_memory(sa__,dacmem2,(uint8_t)(4*(num_lanes+1)),2);
    ...
    if (PRBS_VERBOSE > 2) {
        EFUN_PRINTF(("\n\n\t=====> DEBUG INFO (start) [Core = %d, Lane = %d]\n", osprey7_v2l8p1_acc_get_core(sa__), lane));
        EFUN_PRINTF(("\n\t << Frame Errors accumulated in the DAC Pattern Memory >>\n"));
        EFUN_PRINTF(("\t FEC frames with    0 Errors = %u\n",err_analyzer->prbs_frames_all));
        for (i=0; i<OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i++) {
            if (i < (OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1)) {
                EFUN_PRINTF(("\t FEC frames with   %2d Errors = %u\n",i+1,err_analyzer->prbs_errcnt[i]));
            }
            else {
                EFUN_PRINTF(("\t FEC frames with > %2d Errors = %u\n",(OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1),err_analyzer->prbs_errcnt[i]));
            }
        }
    }

    /* Calculating cumulative error count for respective values of "t" */ 
    for (i=(OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1); i >= 0; i--) {
        if (i!=(OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1)) {
...
err_code_t osprey7_v2l8p1_get_prbs_error_analyzer_err_count(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    ...
    ESTM(err_analyzer->pp_field2 = osprey7_v2l8p1_acc_get_lane(sa__)); 
    EFUN(osprey7_v2l8p1_INTERNAL_get_num_bits_per_ms(sa__, 1, &err_analyzer->pp_field1));
    ...
    EFUN(osprey7_v2l8p1_get_prbs_error_analyzer_lane_err_count(sa__, err_analyzer, lane));

    return (ERR_CODE_NONE);
}

/* Get PRBS Error Analyzer Aggregate Error Count */
err_code_t osprey7_v2l8p1_get_prbs_error_analyzer_aggregate_err_count(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(osprey7_v2l8p1_get_prbs_error_analyzer_lane_err_count(sa__, err_analyzer, num_lanes)); /* For aggregate counters, just pass the num_lanes to point to the correct memory location in the DAC */

    return (ERR_CODE_NONE);
}

static err_code_t _osprey7_v2l8p1_display_fec_frame_error(srds_access_t *sa__, uint8_t errs_per_frame, uint32_t num_frame_errs) {
...
err_code_t osprey7_v2l8p1_display_prbs_error_analyzer_err_count(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    uint8_t i;

    if (err_analyzer->diag_mode)
    {
        for (i=0; i<(OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1); i++) {
            EFUN_PRINTF(("%08x,", err_analyzer->prbs_errcnt[i]));
        }
        EFUN_PRINTF(("%08x\n", err_analyzer->prbs_errcnt[OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1]));
        EFUN_PRINTF(("%08x\n", err_analyzer->prbs_frames_all));
    }
    else
    {
        ...
        for (i=0; i<OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i++) {
            EFUN(_osprey7_v2l8p1_display_fec_frame_error(sa__, i, err_analyzer->prbs_errcnt[i]));
        }
        EFUN_PRINTF((" -------------------------------------------------------------\n"));
    }
...
err_code_t osprey7_v2l8p1_display_prbs_error_analyzer_aggregate_err_count(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    uint8_t i;
...
err_code_t osprey7_v2l8p1_display_prbs_error_analyzer_struct(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer, uint32_t timeout_s) {
    INIT_SRDS_ERR_CODE
    if (err_analyzer->diag_mode)
    {
        /* Start of data dump */
    ...
    /* Display PRBS Error Analyzer Config */
    EFUN(osprey7_v2l8p1_display_prbs_error_analyzer_config(sa__, err_analyzer, timeout_s));
    /* Display PRBS Error Analyzer Err_Counts */
    EFUN(osprey7_v2l8p1_display_prbs_error_analyzer_err_count(sa__, err_analyzer));
    if (err_analyzer->diag_mode)
    {
        /* End of data dump */
...
err_code_t osprey7_v2l8p1_prbs_error_analyzer_aggregate_config(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer) {
    INIT_SRDS_ERR_CODE
    uint8_t num_lanes, lane;
    /* uint8_t aggr_mode; */
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer->lane_mask_aggregate >> lane) & 0x1) {
            EFUN(osprey7_v2l8p1_acc_set_lane(sa__,lane));
            EFUN(osprey7_v2l8p1_prbs_error_analyzer_config(sa__, err_analyzer));  /* Configure PRBS Error Analyzer */
...
err_code_t osprey7_v2l8p1_display_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, uint32_t timeout_s) {
    INIT_SRDS_ERR_CODE
    ...
    ESTM(lane_mask_aggregate = (uint8_t)(0x1 << (osprey7_v2l8p1_acc_get_lane(sa__))));

    /* Call the aggregate function to display error analyzer projections */
    EFUN(osprey7_v2l8p1_display_aggregate_prbs_error_analyzer_proj(sa__, prbs_err_fec_size, SRDS_OSP_100GE, lane_mask_aggregate, timeout_s, 0x0));
    return (ERR_CODE_NONE);
}

/* Display Aggregate PRBS Error Analyzer Projection */
err_code_t osprey7_v2l8p1_display_aggregate_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, enum srds_prbs_error_analyzer_aggregate_mode_enum prbs_err_aggregate_mode, uint8_t lane_mask_aggregate, uint32_t timeout_s, uint8_t diag_mode) {
    INIT_SRDS_ERR_CODE
    ...
    ENULL_MEMSET(&err_analyzer, 0, sizeof(osprey7_v2l8p1_prbs_err_analyzer_config_status_st));
    ...
    EFUN(osprey7_v2l8p1_prbs_error_analyzer_reset(sa__));  /* Reset PRBS Error Analyzer and clear DAC */
    EFUN(osprey7_v2l8p1_prbs_error_analyzer_aggregate_config(sa__, &err_analyzer));  /* Configure PRBS Error Analyzer Aggregation Mode*/
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer.lane_mask_aggregate >> lane) & 0x1) {
            EFUN(osprey7_v2l8p1_acc_set_lane(sa__,lane));
            EFUN(osprey7_v2l8p1_prbs_error_analyzer_start(sa__));  /* Start PRBS Error Analyzer */
        }
    }

    EFUN(osprey7_v2l8p1_INTERNAL_seconds_to_displayformat(timeout_s, &hrs, &mins, &secs));
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer.lane_mask_aggregate >> lane) & 0x1) {
            ...
            EFUN(osprey7_v2l8p1_get_prbs_error_analyzer_err_count(sa__, &err_analyzer));
            /* Display PRBS Error Analyzer Config */
            EFUN(osprey7_v2l8p1_display_prbs_error_analyzer_struct(sa__, &err_analyzer, timeout_s));
            EFUN(osprey7_v2l8p1_prbs_error_analyzer_compute_proj(sa__, err_analyzer, timeout_s));
        }
    }
    if (err_analyzer.num_of_aggregate_lanes > 1)
    {
       ...
       err_analyzer.prbs_frames_all = 0;
       EFUN(osprey7_v2l8p1_get_prbs_error_analyzer_aggregate_err_count(sa__, &err_analyzer));
       EFUN(osprey7_v2l8p1_display_prbs_error_analyzer_aggregate_err_count(sa__, &err_analyzer));
       EFUN(osprey7_v2l8p1_prbs_error_analyzer_compute_proj(sa__, err_analyzer, timeout_s));
    }
...
/*----------------------------------------------------------------------------------------------------------------*/
err_code_t osprey7_v2l8p1_prbs_error_analyzer_report_proj(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st err_analyzer, uint32_t timeout_s, osprey7_v2l8p1_prbs_err_analyzer_report_st *err_analyzer_report) {
#ifdef SERDES_API_FLOATING_POINT
    INIT_SRDS_ERR_CODE
    uint8_t    delta_n;
    uint8_t     i;
    ...
    USR_DOUBLE meas_fec_ber[OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS];
    ...
    ENULL_MEMSET(err_analyzer_report, 0, sizeof(osprey7_v2l8p1_prbs_err_analyzer_report_st));
    EFUN(osprey7_v2l8p1_INTERNAL_get_num_bits_per_ms(sa__, 1, &num_bits_per_ms));

    if (err_analyzer.fec_size_frac == OSPREY7_V2L8P1_PRBS_FEC_SIZE_FRAC_1_8)
        overlap_factor = 8;
    else if (err_analyzer.fec_size_frac == OSPREY7_V2L8P1_PRBS_FEC_SIZE_FRAC_1_4)
        overlap_factor = 4;
    else if (err_analyzer.fec_size_frac == OSPREY7_V2L8P1_PRBS_FEC_SIZE_FRAC_1_2)
        overlap_factor = 2;
    else
        overlap_factor = 1;
    ...
    start_idx = 0;
    stop_idx  = 0;
    if (err_analyzer.prbs_errcnt[OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1] != 0xFFFFFFFF) {
        stop_idx  = OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS-1;
    }
    for (i=OSPREY7_V2L8P1_PRBS_NUM_OF_ERROR_ANALYZER_COUNTERS; i > 0; i--) {
        if (err_analyzer.prbs_errcnt[i - 1] != 0xFFFFFFFF) {
    ...
    if (delta_n >= 2) {                                                              /* There are at least 2 points to trace a line for extrapolation */
        for(i = start_idx; i < stop_idx; i++) {
            x    = (USR_DOUBLE) (i);
            y    = pow((-log10(meas_fec_ber[i])),OSPREY7_V2L8P1_PRBS_ERR_ANALYZER_LINEARITY_ADJUST);
    ...
    if (delta_n >= 2) {
        ...
        proj_ber[15] = pow(proj_ber[15],1.0/OSPREY7_V2L8P1_PRBS_ERR_ANALYZER_LINEARITY_ADJUST);
...
err_code_t osprey7_v2l8p1_prbs_error_analyzer_compute_proj(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st err_analyzer, uint32_t timeout_s) {

#ifdef SERDES_API_FLOATING_POINT
    INIT_SRDS_ERR_CODE
    osprey7_v2l8p1_prbs_err_analyzer_report_st err_analyzer_rep;
...
err_code_t osprey7_v2l8p1_get_rx_tuning_status(srds_access_t *sa__, uint8_t *status) {
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p1_config_BER_HW_mode(srds_access_t *sa__, uint16_t time_ms, struct osprey7_v2l8p1_ber_data_st *ber_data, struct osprey7_v2l8p1_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    INIT_SRDS_ERR_CODE
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    uint8_t clk_gate = 0;
    ...
    EFUN(osprey7_v2l8p1_INTERNAL_prbs_chk_cdr_lock_lost(sa__, &ber_data->cdrlcklost));

    /* save prbs hardware timer config registers and prbs mode */
    EFUN(osprey7_v2l8p1_get_prbs_chk_hw_timer_ctrl(sa__, prbs_chk_hw_timer_ctrl));

    /* Configure hardware timer to count prbs errors */
    EFUN(osprey7_v2l8p1_config_prbs_chk_hw_timer(sa__, time_ms, &prbs_chk_hw_timer_ctrl->time_ms_adjusted_bak));

    EFUN(osprey7_v2l8p1_prbs_err_count_state(sa__, &err_cnt, &lcklost)); /* Clear prbs error counters and start timer */
    /* Extra delay to account for the synchronization delay in prbs checker */
    EFUN(osprey7_v2l8p1_INTERNAL_hw_timer_sync_delay(sa__));

    return(ERR_CODE_NONE);
}

err_code_t osprey7_v2l8p1_get_BER_data_HW_mode(srds_access_t *sa__, struct osprey7_v2l8p1_ber_data_st *ber_data, struct osprey7_v2l8p1_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) {
    INIT_SRDS_ERR_CODE
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    ...
    /* restore prbs hardware timer config registers */
    EFUN(osprey7_v2l8p1_set_prbs_chk_hw_timer_ctrl(sa__, prbs_chk_hw_timer_ctrl));
